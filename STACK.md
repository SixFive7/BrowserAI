...
| Node runtime | **v24.19.0 LTS**, `node.exe` only | v26 is Current, not LTS, and its `node.exe` is 10 MB larger. `[ASSUMED]` That Node v26's `node.exe` is 10 MB larger. **A size difference carried with no date and no measurement**, deciding which runtime ships. Settle it by downloading both and reading the two file sizes. *Tagged 2026-09-23; the list and the predicate are in `TODO.md`.* |
...
| Snapshots | `Verify.TUnit` (latest; **31.28.0** as of 2026-07-31) | Exact parity with `Verify.XunitV3` -- same monorepo, same release, and Verify's own repo carries *more* test projects for the TUnit integration than the xUnit v3 one. `[ASSUMED]` **Two rows of this table justify components NOTHING IN THE TREE REFERENCES** -- this one and the external-smoke row below it. A justification for a component that is not there cannot be wrong in a way anybody notices, which is what makes it worth naming. Settle it by referencing them or by deleting the rows. *Tagged 2026-09-23; the list and the predicate are in `TODO.md`.* |
