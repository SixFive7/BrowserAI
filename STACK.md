...
see [Versioning policy](DECISIONS.md#versioning-policy-everything-floats-the-build-freezes-it).
...
| Node runtime | **v24.21.0 LTS**, `node.exe` only | v26 is Current and not LTS, and its `node.exe` is **10.62 MiB larger**. Measured 2026-09-23: nodejs.org/dist/index.json gives v26.10.0 with `lts: false` and v24.21.0 as `Krypton`; `node.exe` read out of each official win-x64 archive is 104,714,056 B against 93,580,104 B, a delta of 11,133,952 B, and the payload's own `node.exe` is that second figure to the byte. *Corrected 2026-09-23 (previously "**v24.19.0 LTS** ... its `node.exe` is 10 MB larger")*, which named a version two releases behind what ships and a size difference nobody had measured. |
| Job objects | Hand-rolled `[LibraryImport]` | No credible NuGet wrapper exists -- the candidates have <6K downloads and the newest was published in 2017. `dotnet/runtime` [#126273](https://github.com/dotnet/runtime/issues/126273) proposed built-in support and was closed as not planned. ~60 lines. **A "switch to `CsWin32` once a seventh Win32 API is needed" threshold was STRUCK on 2026-08-17.** It was set before any code existed, was passed almost immediately, and stood at 41 declarations across 9 files (83 across 18 counting `tests/`) without anyone acting on it. A verified comparison of CsWin32, Vanara, TerraFX and `dotnet/pinvoke` was run on 2026-08-17 and **the hand-written declarations beat all four**; Vanara is disqualified outright (its helpers emit 32 ILC diagnostics and die at runtime under AOT). CsWin32 *is* adopted, but **test-only**, as a layout oracle for these structs -- `tests/BrowserAI.Tests/InteropLayoutTests.cs`. *Corrected 2026-09-24 (previously "The full decision, the table and the open licence question are in [`TODO.md`](TODO.md).")*: that item was deleted on 2026-09-23, and the licence question was settled on 2026-09-24 in the direction that lets generated code ship; the decision is the `Generated code` row of [`DECISIONS.md`](DECISIONS.md). |
...
| Tests | **TUnit** (latest; 1.65.0 as of 2026-08-13) | MIT, source-generated, reflection-free, **MTP-native**. Already the house standard in a sibling project. 1.0 shipped 2025-11-05; ~623K downloads/mo and growing 2.24× YoY. Chosen over xUnit v3 because [we do not vendor the SDK's fixtures](TESTING.md#we-write-our-own-harness) -- that was xUnit's only argument here. |
| Snapshots | `Verify.TUnit` (latest; **31.28.0** as of 2026-07-31) | Exact parity with `Verify.XunitV3` -- same monorepo, same release, and Verify's own repo carries *more* test projects for the TUnit integration than the xUnit v3 one. **Chosen and never taken up, measured 2026-09-23:** of the files git says this repository holds, the ones naming `Verify.TUnit` are three and the ones naming `inspector` are four, and every one is a document or a comment -- neither appears in a project file, a lock file, a build script or a CI definition, against a control in which `TUnit` appears in `Directory.Packages.props`, the test `.csproj` and its lock. This row records what would be used if snapshot testing (or an external smoke check) were ever built, and nothing uses it today. |
| Assertions | TUnit built-ins | `await Assert.That(actual).IsEqualTo(expected)`. **Never add FluentAssertions** -- it relicensed at exactly 8.0.0 to a bespoke non-SPDX licence with a commercial tier. A sibling project carries the identical prohibition for the identical reason. |
| External smoke | `@modelcontextprotocol/inspector` **2.2.0** | Language-independent CI check. Exit code **5** means the tool reported `isError` -- the signal `claude mcp` does not give you. **Chosen and never taken up, measured 2026-09-23:** of the files git says this repository holds, the ones naming `Verify.TUnit` are three and the ones naming `inspector` are four, and every one is a document or a comment -- neither appears in a project file, a lock file, a build script or a CI definition, against a control in which `TUnit` appears in `Directory.Packages.props`, the test `.csproj` and its lock. This row records what would be used if snapshot testing (or an external smoke check) were ever built, and nothing uses it today. |
...
directly contradicts [§Windows process spawning](DECISIONS.md#windows-process-spawning):
...
([rule 1](DECISIONS.md#the-five-rules-that-make-floating-safe)). That only works if
...
[why-reason 2](DECISIONS.md#2-the-version-chain-floats-and-it-breaks-silently) with
