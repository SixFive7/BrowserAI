...
> **Built 2026-08-16 ([step 15](build-order.md#15-first-run-provisioning-and-browserai_reinstall_browser)), and one thing the table does not say turned out to be load-bearing.** "Refuses if any session has a live browser" needs a definition of *live* that reaches **other processes**, and neither the in-process session dictionary nor the session index alone provides one: a BrowserAI that died holding a session leaves a browser that no live lock claims. The implementation therefore asks two questions — which processes are running out of the browser tree (by full **image path**, never by image name), and which sessions on this machine are open (this process's own, plus index entries whose `lock.json` holder is alive by pid *and* creation time). Live browsers with a live session are refused naming the sessions; live browsers with none are [row 13](#h4-the-error-catalogue), reported and never terminated. **And the delete is the real guard rather than the check**: Windows refuses to remove a directory holding open executables, so a session that opens a browser between the check and the delete produces a refusal listing exactly which files survived, not a corrupted tree.
...
| 6 | Provisioning in progress | *"First use of this browser version on this machine. The download has started (203.8 MB). Wait about 10 seconds and retry."* — ⚠️ **corrected 2026-08-16:** it also has to say that **every** browser tool is refused meanwhile, `browser_get_config` included. That tool resolves the browser executable before answering ([measured](../kb/playwright/configuration.md#browser-provisioning)), so [§A](A-runtime.md#first-run-browser-provisioning)'s promise that it keeps working was wrong. What does keep working is BrowserAI's own tools, and the row names them |
