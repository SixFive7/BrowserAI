...
- **an empty browsers directory must FAIL this layer.** Without this the entire batteries-included premise can be silently dead code with the suite green. ✅ **Built 2026-08-16 as `SuiteEnvironment`**, and the same argument was found to apply to the published slice and the payload: every capability guard reports as **skipped** rather than as a pass, and as a **failure** under `BROWSERAI_RELEASE_RUN=1`. What made it urgent was a measurement — at `c21fea7`, with the whole publish directory moved aside, the suite reported `329 total · 328 succeeded · 1 skipped · exit 0`, character for character what a run against a real browser reported
