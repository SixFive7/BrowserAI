...
> ⚠️ **Corrected 2026-08-16 @ `@playwright/mcp` 0.0.79 (previously "`browser_get_config` still works" while the download runs).** It does not. Measured twice against the child directly with an empty browsers root: the tool answers `isError: true` from `throwIfExecutableMissing` — it never *launches* a browser, which is why the round trip is cheap on a provisioned machine, but the executable has to exist. So **every** upstream tool is refused until the install lands, this one included, and letting it through would have been worse than refusing it: the caller would get upstream's advice to provision a browser that is already being provisioned, instead of the row-6 refusal that names the size and says the same call will work shortly. What keeps a downloading session inspectable is BrowserAI's **own** tools — `browserai_list`, `browserai_resume` and `browserai_set_purpose` all answer throughout, because none of them needs a browser. [kb](../kb/playwright/configuration.md#browser-provisioning), [row 73](../kb/re-verification.md).
