...
| Browsers | **Not in the installer.** [Provisioned on first run](#first-run-browser-provisioning) — the redistribution position for Chrome for Testing is unresolved and the only on-point public statement is adverse. | 203.8 MB down, 430.48 MiB on disk, ~12.6 s on first run |

**Installer payload: ~117 MB installed** — `node.exe` 88.53 MB + the JS tree 18.11 MB + BrowserAI 9.76 MiB. **Browsers add 203.8 MB down / 430.48 MiB on disk on first run** (re-measured 2026-08-16), once per machine rather than once per update, so **disk after first run is 546.88 MiB ≈ 573 MB**. Per-component provenance in [kb: component sizes](../kb/playwright/provisioning-and-timings.md#component-sizes).

> ⚠️ **This table previously listed the browsers as bundled, totalling ~806 MB installed / ~239 MB compressed.** That was the design before 2026-08-14, when provisioning moved to first run; the row survived the decision and contradicted it for a day. The old figures are retained in [kb: component sizes](../kb/playwright/provisioning-and-timings.md#component-sizes) because a bundled build is the fallback if the redistribution question is ever resolved favourably. **They are not the numbers for *disk after first run***, which this note previously claimed: the ~806 MB total counts `chrome-headless-shell` (268.49 MB), which [is not provisioned at all](../README.md#settled-2026-08-15). Disk after first run is the 116.40 MiB payload plus 430.48 MiB of browsers — **546.88 MiB ≈ 573 MB**, not 806. (Both halves of that sum were re-measured 2026-08-16; the browsers figure was 433 MiB and the ≈ 570 MB it produced was [right by way of two cancelling unit conflations](../kb/playwright/provisioning-and-timings.md#first-run-provisioning).)
...
**Measured 2026-08-15 and re-measured 2026-08-16, exact `content-length` from the CDN:** `chrome-win64.zip` 202,283,919 B + `ffmpeg-win64.zip` 1,411,741 B + `winldd-win64.zip` 128,684 B = **203.8 MB down, 430.48 MiB on disk**, byte-identical across both dates at chromium rev 1237. Arithmetic for slower links: 2 m 43 s at 10 Mbps, 27 m 11 s at 1 Mbps. End to end it takes **12.6 s and 12.0 s** on a ~300 Mbps link, measured twice into an empty root.

> ⚠️ **Corrected 2026-08-16 (previously "433 MiB on disk").** The old figure was three rounded components added together; the measured total is **430.48 MiB** — 451,389,780 B across 318 files. [kb](../kb/playwright/provisioning-and-timings.md#first-run-provisioning) carries the per-directory breakdown and how to re-establish it.

> ⚠️ **The mirror rotation does not cover Chromium.** Read 2026-08-16 in `playwright-core`: Chrome for Testing resolves through `cftUrl`, whose mirror list is **one host** (`cdn.playwright.dev`, with no `/dbazure/download/playwright` prefix) — so the five retries all hit the same name, and the rotation that justifies stripping `PLAYWRIGHT_DOWNLOAD_HOST` protects only `ffmpeg`, `winldd` and `firefox`. The strip is still right; "five attempts at five hosts" was never true of the 202 MB half. [kb](../kb/playwright/provisioning-and-timings.md#first-run-provisioning), [row 72](../kb/README.md#re-verification-index).
...
> ⚠️ **Corrected 2026-08-16 @ `@playwright/mcp` 0.0.79 (previously "`browser_get_config` still works" while the download runs).** It does not. Measured twice against the child directly with an empty browsers root: the tool answers `isError: true` from `throwIfExecutableMissing` — it never *launches* a browser, which is why the round trip is cheap on a provisioned machine, but the executable has to exist. So **every** upstream tool is refused until the install lands, this one included, and letting it through would have been worse than refusing it: the caller would get upstream's advice to provision a browser that is already being provisioned, instead of the row-6 refusal that names the size and says the same call will work shortly. What keeps a downloading session inspectable is BrowserAI's **own** tools — `browserai_list`, `browserai_resume` and `browserai_set_purpose` all answer throughout, because none of them needs a browser. [kb](../kb/playwright/configuration.md#browser-provisioning), [row 73](../kb/README.md#re-verification-index).
