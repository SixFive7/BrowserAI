...
| BrowserAI itself | NativeAOT, single file, no .NET runtime on the host. | **9.76 MiB**, measured by the [AOT spike](#nine-places-where-the-sdk-must-be-deviated-from) |
...
>
> BrowserAI depends on **nothing** from the host — no Node, no .NET runtime, no Google Chrome. All three modes set `browserName: "chromium"` **and an explicit chromium-alias channel**, never `channel: "chrome"`, so every mode runs the Chromium BrowserAI provisioned rather than the user's Chrome. Dropping the channel *entirely* is not the same thing and does not work — see [config generation](#config-generation-and-validating-it-against-the-runtime-we-ship). Ship as NativeAOT single-file.
...
- **Every tool schema gains a required `session` parameter** (the session directory — see [§H](#h-the-model-facing-surface) for why it is not called `handle`), injected by BrowserAI into the raw `inputSchema` of **every tool in the surface — somewhere between 24 and 69, and not the same number in every mode.** 69 is the maximum ever exposed over MCP, not ours: 24 is the default with no capabilities set, we add `vision`, `devtools` and `config` everywhere and `storage` (17 tools) on `persistent` alone, and [the per-capability breakdown was never counted](kb/playwright/tools-and-artifacts.md#the-tool-surface-and-the-package-shape) — so the real figure is a bracket until someone counts it from the resolved bundle. Do this on the `JsonNode`; never materialise a typed schema to do it, because the typed path silently discards unknown tool-level members. Keep the injection order-stable — the spec SHOULDs deterministic tool ordering for prompt-cache hit rates.
...
<session-dir>/
  ...
  output/        <- --output-dir, holding the typed folders of [§F](#f-artifact-management)
...
| Handle routing and instance lifetime | Injecting `session` into every schema in the surface — 24–69 of them, varying by mode — can perturb tool ordering, and an unstable rewrite quietly costs prompt-cache hits on every session | [kb: the 2026-08-15 spike](kb/mcp/sdk.md#measured-by-spike-2026-08-15) · [§C](#c-the-init-tool-and-instance-handles) |
...
| Bundling and AOT | Killed children leak `browser@<guid>` descriptors into the browsers-registry root at `%LocalAppData%\BrowserAI\browsers\` — which [survives every update by design](#a-ship-and-own-the-runtime), so the leak **accumulates across releases** instead of being wiped by one | [kb: artifacts](kb/playwright/tools-and-artifacts.md#artifacts-and-output-directory-behaviour) · [§A](#a-ship-and-own-the-runtime) |
...
| Child runtime and configuration | Outer browser dirs use underscores, inner ones dashes — `chromium-1237\chrome-win64\`, and for the shell we do not provision, `chromium_headless_shell-1237\chrome-headless-shell-win64\`, so a path built consistently is wrong | [kb: first-run provisioning](kb/playwright/provisioning-and-timings.md#first-run-provisioning) · [§A](#a-ship-and-own-the-runtime) |
...
| Child runtime and configuration | **Running out of disk mid-provision is success-shaped.** Provisioning needs 203.8 MB down and 433 MiB extracted, ~640 MiB at peak; a download that dies partway leaves the `spawn EFTYPE` shape — stderr empty, `initialize` and `tools/list` both fine, discovered at first navigation. Check free space at `init`, and only with an **O(1) volume query — never a directory walk**, which would make the check slower than the failure it prevents | [kb: browser provisioning](kb/playwright/configuration.md#browser-provisioning) · [§C](#the-init-contract) |
...
| Packaging and updates | Desktop and Start Menu shortcuts are created **by default** (`--shortcuts` defaults to `Desktop,StartMenuRoot`), and `%LOCALAPPDATA%\velopack\` is created unconditionally and **not removed by uninstall** | [kb: Velopack](kb/packaging/velopack.md) · [§G](#g-updates) |
