...
foreach ($cap in @(4636, 4635, 4634, 4632, 4630, 4626)) {
    & $pwshPath -NoProfile -NonInteractive -File $script -Mode Controller -Desktop ('BaiF' + $cap) -TitleChars 2048 -FixedCap $cap -Controls 0 -Reps 3 -BrowserWaitMs 8000 -Out (Join-Path $out ('fine-' + $cap)) 2>&1
}
...
& $pwshPath -NoProfile -NonInteractive -File $script -Mode Controller -Desktop 'BaiSmall' -TitleChars 0 -Controls 0 -Reps 5 -MaxFillers 10 -BrowserWaitMs 8000 -Out (Join-Path $out 'small-windows') 2>&1
