...
**Chromium only, today.** `browserai_init` refuses `browser: "firefox"`; the machinery behind Firefox is built and measured, and what is still owed to offer it is in [`TODO.md`](TODO.md).
...
461 executed test cases, 0 failed, 0 skipped — measured from a full `dotnet test` run on 2026-08-18 (*previously "458"*, and "436" and "419" before that; re-measured each time rather than adjusted — the last move added the three arms that cover the restored inline screenshot: one off the wire against the real child, one over the guard's other side, and one for a file that cannot be read back). **The predicate is executed cases, not `[Test]` methods**: every `[Arguments]` and `[MethodDataSource]` expansion counts as one, which is why no test in the suite asserts this number — reflecting over attributed methods would answer a different question, and a count that reads like a measurement and is not is the thing this repository spends the most effort avoiding. The publish emits a NativeAOT binary with ILC reporting nothing, and the update feed resolves over real HTTP with a manifest whose SHA-256 matches the package byte for byte.
