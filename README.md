...
**Three modes**, bound at `init` and recorded in the session's `browserai.json`. A mode is two switches on the browser BrowserAI launches for that session — whether a window appears, and whether the cookie and storage **tools** exist in that session's child:

| Mode | Window | Cookie and storage tools |
|---|---|---|
| `headless` | No | No — the workhorse |
| `interactive` | Yes | No — a human can sign in and the agent cannot ask this server for the cookie jar |
| `persistent` | Yes | Yes — logged-in agent work |

A session without them has its child launched **without upstream's `storage` capability**, so the 17 cookie, `localStorage` and `storageState` tools do not exist in that process at all.

⚠️ **Corrected 2026-08-19 (previously the third column was "Stored credentials", reading No / No / Yes, and `interactive` was described as "a human may type a password the agent must never capture").** **All three modes persist.** BrowserAI writes `browser.userDataDir` as `<session>\profile` for every mode and never writes upstream's `isolated` key, so cookies and `localStorage` survive a `browserai_resume` in a `headless` session exactly as they do in a `persistent` one — and they survive on disk until the session directory is destroyed. `storage` is a **tool filter, not a persistence switch**. The correction points the safe way: a caller who believed a mode discarded credentials would leave a signed-in profile behind thinking it had not, and `browserai_destroy` is the thing that removes it.
