...
870 executed test cases, 0 failed, 0 skipped -- measured from the **two-shell gate** of 2026-09-24 at `fcdb1c1`: one full `dotnet test` run from PowerShell forcing `C:\` at 3m 44.0s and one from Git Bash forcing `c:\` at 3m 22.3s, both `FULL RUN`, both publish-`FRESH` with the binary and the tree both at 1.1.1-alpha.0.124, both with every capability `PRESENT`, no test on a degraded path, and both `windows` rows `CLEAN` (*previously "853"* from the two-shell gate earlier the same day, *"841"* from the two-shell gate earlier the same day, *"831"* from the two-shell gate earlier the same day, *"817"* from the two-shell gate earlier the same day, *"814"* from the two-shell gate earlier the same day, *"807"* from the two-shell gate earlier the same day, *"788"* from the two-shell gate of 2026-09-23, *"787"* earlier the same day, *"784"* from the six-run release gate of 2026-09-22, *"783"* from the two-shell gate earlier the same day, *"781"* from the first two-shell gate of 2026-09-22, *"779"* from the two-shell gate of 2026-09-21, *"766"*, *"770"* and *"766"* earlier the same day, *"765"* on 2026-09-18 and *"764"* earlier that day, *"762"* from the six-run release gate of 2026-09-17, *"761"* from the two-shell gate earlier the same day, *"755"*, *"750"* and *"747"* earlier the same day, *"741"* from the six-run release gate of 2026-09-16, *"737"*, *"720"*, *"706"*, *"685"*, *"676"* and "675" earlier the same day, "671" earlier still, "652", "651", "650", "647", "644", "643", "644", "641", "640", "637", "626", "624", "613", "604", "634", "648", "625", "622", "618", "614", "603", "601", "596", "593", "589", "585", "582", "576", "573", "571", "551", "548", "532", "531", "530", "514", "505", "506", "500", "501", "498", "497", "495", "493", "491", "478", "476", "461", "458", "436" and "419" before that; re-measured each time and not adjusted). **The +1 is the packer's own asset list, later on 2026-09-23.** One arm in `ReleaseScriptTests` drives `build/Set-UploadAssets.ps1` over the list `vpk pack` really writes and holds that the portable archive is gone from it, that keeping it is what leaves it there, and that a list with no full package is refused. **Planted red twice**: with the filter removed the step refused itself and the arm named nothing, so the by-name assertion was moved ahead of the set comparison and the second red read *"Expected to not contain"* the archive by name. **The +3 is the upload set and the evidence index, 2026-09-23.** Two arms in `ReleaseScriptTests` hold what a release publishes -- that `build/New-Release.ps1` declares the installer, the full package and `releases.<channel>.json` and nothing else, and that every other file a pack leaves behind is classified, not merely absent from the set. Both were planted red: a doctored declaration that published the portable archive again came back naming it, and dropping `RELEASES` from the classified list came back naming that against the real directory. One arm in `HouseRuleTests` holds that every batch under `docs/evidence/` is a row in that directory's own index, in both directions -- **planted red, and it named three batches that were already missing**, the oldest of them two days old. **The +13 is `browserai_page_tool`, 2026-09-21.** Ten arms in `PageToolTests` drive it through the published binary against real pages that really register WebMCP tools -- a happy path whose page echoes back exactly the arguments the caller sent and nothing else, a name the page does not offer, a tab that navigated away, the same name on two pages with `page` given, two tools with one name on one page, a display title that is not the name, a page tool that never answers, and the door still refusing a page-supplied name outright. Three arms in `ErrorCatalogueTests` provoke the six new refusal rows. **Watched red twice, and the second one is the interesting one.** With the routing disabled and everything else in place, 8 of the 10 arms went red on their own first assertion, each reading *"'browserai_page_tool' is not a BrowserAI session tool"*; the two that stayed green are the two that are about existing behaviour, which is the control. Then, with the argument object taken from the caller's whole node instead of its `arguments` member, exactly 1 of the 10 went red -- and what it printed is the defect itself: the page's own handler echoed back `{"session":"C:\Source\...\page-tool-session","name":"Do The Thing!","arguments":{"who":"world"},"why":"the suite exercising this call"}`. **The -4 is four tests RETIRED with the mechanism they guarded, 2026-09-21**, and it is the first time this number has gone down. `PayloadTests.TheDatedPlaywrightCoreOverrideIsStillNeeded`, `.TheExpiryComparisonFiresInBothDirections`, `.TheResolvedPlaywrightCoreIsWhateverTheOverrideSays` and `SessionPolicyTests.TheWebMcpCallIsWithheldOnLivenessAndTheWebMcpListIsNot` were deleted, not skipped. The first three answered *has the dated `playwright-core` override's exit fired*; `@playwright/mcp` 0.0.82 shipped the fix the override was taken for, the `overrides` block went, and a mechanism that no longer exists cannot be tested. The fourth asserted that the WebMCP pair was judged in two directions, and upstream took both tools off the wire, so every premise it rested on is gone. **Deleting a test for a mechanism that no longer exists is not a skip**, and each deletion is named where the test was referenced and not removed without trace -- here, in `TODO.md`, in `DECISIONS.md` and in the source files themselves, which carry a `RETIRED` comment in the place the code stood. What the first three guarded came back to where it was before 2026-09-17: `PayloadTests.TheLockRecordsUpstreamsOwnExactPinOfPlaywrightCore` asserts the resolved `playwright-core` **equals** the declared one again, watched red against a doctored lock, and that is also what catches an override being added back.

**The +17 is phase 1b, later on 2026-09-24, and the gate took three attempts, so every red is named here.**
At `417409a` the PowerShell half was 869 of 870 and the Git Bash half 868 of 870; at `883e347` the PowerShell
half was 869 again; at `fcdb1c1` the PowerShell half was 869, then Git Bash 870 of 870 and PowerShell 870 of
870 on the same commit. **Three reds were the suite's and are fixed, each watched red against a plant first**:
the pipe describe arm read the description the instant the navigate's answer arrived, and a call is counted
until the handler that wrote its answer returns, so it now waits for the count to settle, red against a server
planted to hold the count two seconds; the coverage-block arm classified a commit-charge reading of its own
beside the block's, and at 75.0% of the limit the two fell either side of the band, so it now classifies the
reading the block printed, red with the row planted at 76%; and the orphan-console rig launched the server
through `cmd /c start /b` with nothing ordering cmd's exit before the server's look at it, so the server read
its launcher alive, and the launcher is now the test probe's `launch-suspended` mode, which starts the server
suspended and exits before the rig resumes it, red with a two-second linger planted after cmd's `start` in
both shapes. **Two reds were not fixed and are not this tree's**: a Firefox launch that never finished,
both in runs whose commit charge read `TIGHT` -- once in `PasswordPromptTests`, whose error was Playwright's
own 180 s `initializeServer` timeout, and once in `FirefoxSessionTests`, whose navigate failed after 3m 01.8s
with its text not kept. That is the stuck launch
[provisioning and timings](kb/playwright/provisioning-and-timings.md) records, and the second is read as the
same one from its duration alone. The install hook's new
environment broadcast was tried as a cause and one experiment did not implicate it: the four
Firefox-launching classes ran eight times under a broadcast every 500 ms, 112 of 112 green.
**Five are the user PATH**, in `UserPathTests` (Q294 b): each install root putting its own folder on the PATH
and taking only that off, red with the PATH step planted out; an install and its uninstall leaving the value
byte for byte, red with an append that skipped the separator after a value ending in one; the registry store
keeping the kind and the unexpanded text, red with every write made `REG_SZ`; a bare name judged by the file it
finds first on the PATH, red with the bare-name branch planted out; and a Codex project entry naming the server
alone and saying what it finds, red with the absolute path written back. **Two remove BrowserAI from a project
you pick** (Q289 b), in `ConfigurationAppTests`, driven through the dialog's own dispatch with a picker that
opens nothing -- one holding the picker to its click and its owner and the removal to an entry of ours, one
holding a Codex removal to the project's own home -- both red with the verb left out of the session's dispatch.
**Three are the installer lock** (Q291 a), in `InstallerLockTests`: this run holding the lock, red with the
session hook not taking it; the decision table and the drivers' half went in with the protocol they test and
were not seen red on their own. **Four read the gate or drive its script**, in `SuiteCoverageTests`: every
driver holding the lock for its whole run, red three times per driver; every driver packing the suite's
installer before its run, red once per driver; the clearance comparing only each client's `browserai` entry
(Q292 a), red against the whole-file hash; and the clearance reading the user PATH byte for byte, red against
the script without that reading. **One packs the suite's installer** (Q287 a),
`ReleaseScriptTests.ATestPackOnlyRunPacksTheSuitesInstallerAndItsTwinAndNothingElse`, red against the script
without the mode, and **one holds that installer to this tree's build**,
`RealInstallerTests.TheSuitesInstallerIsTheBuildThisRunTested`, red against a 1.1.0 test pack packed for the
purpose. **One is the Codex classifier**,
`CodexRegistrationTests.ACodexEntrySpelledWithAVariableIsNeverOursBecauseCodexExpandsNothing`, red while the
classifier still expanded the variable. **Arms that changed without moving the count**: the real-installer
arms now hold the per-client registration record and the user PATH, red against a test pack of a planted
build; the pipe stop arm waits for each browser to exit, red with a single read planted before the server's
exit; and `WindowWatchTests` holds that an unwatched run fails in every mode (Q290 a), red against the old
decision.

**The +12 is phase 1 of the coordinator work, later on 2026-09-24, and both halves of the gate were green on the first attempt.**
**Eight are the pipe each server answers on**, in `ServerPipeTests`, and each was watched red against a
planted defect: a published server describing itself, planted red by reporting the publish directory as its
working directory; the pipe's DACL holding one entry for the current user where the default holds more, red
with an `Everyone` entry added; the remote-client flag read from the client's end, red with the flag removed; a second instance and a
name somebody else created first both refused, red with the first-instance flag removed and again with a
second instance allowed; a server dying mid-answer read as no answer through the length prefix, red when a
short body was accepted; a hung listener costing the whole bound while the census still says held, red at a
tenth of the bound and again with the census planted false; a free marker, a marker with no pipe and a pipe
served by another pid, red with the census skipped, with the pid check skipped and with a missing name waited
on; and a stop through the pipe ending a published server the graceful way, red when the stop exited the
process outright and left its instance directory behind. **Three are the update refusal**, in
`UpdateInProgressTests`, against the published binary: a stop with a call in flight answering that call, red
when the stop refused nothing; a server started while its install's updater runs refusing calls, starting no
child and ending with the updater, red with the updater never looked for and again with no watch on it; and
the same server with no updater serving normally, red when every server read as updating. **One is the
catalogue row**, `ErrorCatalogueTests.TheUpdateRowIsEmittedByACallCutOffByAStopAndByACallMadeAfterIt`, red
when a second answer got through the door and again when a call cut off in flight was told nothing had run.

**The +10 is the focus guard and the batch around it, later on 2026-09-24, and both halves of the gate were green on the first attempt.**
**One is the residue scan**, `HouseRuleTests.NoCommentCarriesTheResidueOfTheScriptThatWroteIt`, watched red
naming eight comments a patch script had written its own source into. **Five are the window watch**, in
`WindowWatchTests`: the classifier driven both ways over synthetic sightings; the row and the refusal in
all three states; the premise that this run's hooks went in; a child test host on a private desktop that
shows one window through the probe's new `window-show` mode and must exit 10 naming its class while the
parent never sees it, watched red with the watch's show events planted out; and the planted arm that child
runs. The watch itself was watched red against the real offender first:
`RealInstallerTests.TheInstalledMainExecutableOpensOneDialogAndNoConsoleWindow`, unmodified, in a host on a
private desktop, exited 10 naming its `#32770`, and moved onto a desktop of its own it is green, which moved
no count. **One refuses a real toast in any test**, `HouseRuleTests.NoTestRaisesARealToast`, with its
synthetic control both ways. **One holds the gate's clearance to a parse of `~/.claude.json`**,
`SuiteCoverageTests.TheClearanceSnapshotReadsTheRegistrationWithoutStartingTheClient`, watched red against the
script that started the client. **Two are installer facts made arms**, in `RealInstallerTests`: two roots of one
pack id sharing one Add/Remove key that either uninstall deletes, and an installed server that never sees
the installer's variable beside a copy outside any install that does, each watched red against a planted wrong
expectation. **Three arms were rewritten in place and moved no count**:
`ForbiddenDependencyTests.AProjectUnderSrcThatReferencesTheCodeGeneratorShipsTheMetadatasNotice` replaces the
arm that refused any generated code, watched red naming three metadata packages;
`InstallerHandoffTests.ThePublishedBinaryServesTheClientThatStartedItWithTheInstallersVariableSet` replaces the
arm that required the deleted installer exit, watched red against a build that still carried it; and the
variable arm of `ThePublishedBinaryExitsWhenItsLauncherIsGoneAndStdinIsAConsole` now requires the general exit.

**The +14 is Q258's Codex steps 2 to 6, later on 2026-09-24, in four groups, and both halves of the gate were green on the first attempt.**
**Two are the hooks**, in `RegistrationTests`: one hook registers every client and the record carries an
entry for each, and a client the machine does not have is named in the record without failing the hook --
both planted red by a hook that registered only the first client. **Six are the window and the registrar's
project path**: every registration action belongs to exactly one named client, planted red by one
identifier block for both clients, which also reddened the older `NoCommandCanBeMistakenForTheCloseButton`
on its distinct-identifier check; removing a project registration is offered only where there is one of
ours; and four arms over `McpRegistrar.ApplyToProject` against the double -- the project's home and never
the user's (planted red by an empty environment), a stale entry of ours removed before it is written again
(planted red by deleting the remove-first), another install's entry refused (planted red by skipping the
gate on a remove), and the residue removed only while empty (planted red by the `File.Delete` it was first
written with). The re-register affordance was planted red in the renamed status arm, which is why the
rename moved no count. **One is an uninstall over nothing**, which now runs nothing for either client,
watched red against the code before the fix. **Five drive the real Codex** under a scratch `CODEX_HOME`:
its exit-zero dialect, the user-scope round trip through the registrar, a project registration written
into the repository and nowhere else -- the arm that found every `codex mcp` verb refusing a home that is
not there, which the double then modelled and was watched red on *received Refused* -- the CLI located as
an executable, and the update effect: a BrowserAI registered in Codex serves one call through a real
`codex app-server` and leaves nothing that holds an update, with a HELD control inside the arm. **N16 added
assertions and no arms**: the version half of the freshness reading, planted red by a doctored comparison
and then seen live refusing a binary one version behind, and the reconnect arm's wire version, planted red
by a shim that did not record it.
...
**The +3 is T7, later on 2026-09-24: the reap at session close.** **One arm drives the whole thing**:
`ServerRegistryReapTests` opens two sessions with a real Chromium each through the published binary,
plants a thousand dead descriptors in a scratch registry, destroys one session, finds the reaper's pid and
creation time in the machine's process log, waits for that process to exit, and then holds that every dead
descriptor is gone, that the closed session's own went with its browser, and that the live browser's still
stands. **Planted red**: with the teardown's reap made unreachable and the slice republished, it failed on
*the close should have recorded the reaper it started* -- 1 failed, 1 succeeded -- and green 2 of 2 with the
call restored. **A second arm holds the script** -- upstream's `serverRegistry.list()` against the module it
was given, forward slashes, an escaped apostrophe, and no `cli-client` anywhere in it -- **and a third, in**
`ChildEnvironmentTests`, **holds the one forwarded test hook** both ways: inherited when this process carries
`PWTEST_SERVER_REGISTRY`, absent when it does not, refused to nobody.

**And one assertion was removed the same day, with no count behind it**, because it was a promptness claim in
disguise: *the reaper was still running when the destroy answered* holds only while the test host observes
the answer inside the reap's own window, and at full parallelism this host is starved for seconds -- it
passed alone and failed in the suite with the product behaving identically. What survives is the ordering
read out of the log. **The same gate surfaced a stray the suite had been handing the sweep for as long as
that file existed**: `PasswordPromptTests` took a session's guard and released it on the spot, so the
browser it launched into that profile was a stray by the product's own definition, and the sweep's record
names the pid the arm's own Playwright log carried. The guard is held for the child's life now, **planted
red and watched in both directions under one real `--sweep` a second**: 3 of 3 red with it dropped, 3 of 3
green with it held.
