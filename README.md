...
**A reinstall takes the machine's browsers root for the whole call, and `browserai_init` and `browserai_resume` are refused while it holds it.** *Added 2026-08-19.* Two agents on one machine could otherwise race: the reinstall establishes that nothing is running out of the tree, and the other one's `init` launches a browser into that tree while the delete is part way through it — the check was right when it was asked. The claim is a lock file at the root of the browsers directory, so a process that dies releases it; a second reinstall is refused by it too. **There is no drain and no wait.** A reinstall that finds sessions already open refuses at once and says how many are running; it never holds the machine waiting for a browser a human may never close.
