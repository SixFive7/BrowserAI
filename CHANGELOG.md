...
for what the entry *is*, not for the group it sits under, so a fix that
...
- ✨ **A web page's own tools are callable, through one tool of ours that is judged like the rest.**
  ...
  **Page tools are reached as a CLASS, not through rows, and that is the decision.**
  ...
  **Bounded at 60 seconds, by BrowserAI, because nothing upstream bounds it at all.**
  ...
  between about half a second and never. **It is a real recovery, not a give-up, and
...
- ✅ **The scratch configuration the suite hands the real client is seeded as already onboarded.**
  ...
  own throwaway-config recipe**, read out of the shipped bundle and not invented here --
...
- ✅ **A log event id must be unique in its class, and the scan found a second collision on its first run.**
  ...
  list is read out of the comment and not typed into the test: a second copy is a
  ...
  **The marker parse is held in both directions**, because the same comment contains a
  ...
  id is scoped to its class), and the marker reading a list, not a sentence.
...
- 🐛 **Two client-liveness events shared event id 76, so both moved off it and the id is retired.**
  ...
  `git log -S` and not remembered. `ClientWaitCannotBeInterpreted` is 77 now and
  ...
  recorded in the code and not closed. **Retiring the id is what stops it spreading:**
  ...
  **This is the maintainer's decision and not the repair that was first offered.** The
  ...
  class as of this release, not out of one file.
...
- 📦 **Every release packs full packages only, so an update downloads the whole thing.**
  ...
  **The mode was resolved from the tool, not from memory**: `vpk pack --help`
  ...
  **What it costs is measured and not waved at**: a same-tree delta is **138,791 b**
  ...
  working, not `vpk` having nothing to compare against.

  Four documents were re-scoped and not left to rot, each with what it previously said:
...
- 🔧 **The relaunch note stops promising that stored state survived, because it may not have.**
  ...
  **`DeadChildTests.TheRelaunchNoteDoesNotPromiseThatStoredStateSurvived` is the guard**,
  and it is a guard on the text, not on the constant. Every other arm in that file
...
- ⬆️ **Velopack rolled 1.2.0 to 1.2.158, and the Windows installer is a 64-bit bootstrapper now.**
  ...
  the gate found it, not because anybody predicted it -- see the stub naming below.

  **What arrives with it and is visible to a user.** `Setup.exe` and the root stub are
  built x64 for a win-x64 pack, not i686, measured on the packed artifact as PE
  ...
  **The one thing in this tree that had to move, and the gate is what found it.** Upstream
  ...
  not on the title wherever it appears -- stripping `BrowserAI` from names would
  ...
  **The rename arrives in a second place, and the same arm found that on the next run.**
  ...
  not nondeterminism and it was checked and not assumed** -- every shipping pack on this
...
- 📝 **The feed is asked once per server start, and a hundred browsers in one session ask nothing.**
  ...
  **The monitor is the asset's own download counter and it discriminates.** On 2026-09-22
  ...
  monitor, not the service. The alternative offered and declined was a machine-wide
  check stamp in the shared data root.

- 🔧 **`webmcp: true` is written into every generated config and not left to upstream's default.**
  ...
  upstream's default moves this is a red build, not a capability that quietly went.
...
- 📝 **Dash style for the whole repository is recorded as an open question and not settled.**
  ...
  two styles by accident, not by anybody's decision.
...
- 🔧 **A session's deletion, what a destroy takes, its upload root and its move are all said now.**
  ...
  caller reads as a claim about the session, not about the screenshots it was asked
  to produce. `browser_file_upload` can only reach a file inside the session's `output`
  folder, which is this project's `allowUnrestrictedFileAccess: false`, not
...
- ✅ **Four arms hold the session-lifetime rules on the published binary's own wire.**
  ...
  OFF THE WIRE, NOT OFF THE CONSTANTS, for this file's standing reason: these
  ...
  against. Phrases, not whole sentences, because the wording is not the maintainer's
...
- 💥 **The payload could not be rebuilt until the override's exit was adjudicated.**
  ...
  retracted, and the answer was to roll and not to widen the comparison.
  ...
  epoch-milliseconds alpha, not the `alpha-YYYY-MM-DD` shape upstream publishes
  ...
  not reasoned. 1789764292000 ms is 2026-09-18T20:44:52Z; that `playwright-core` was
  ...
  instead of matching everything. The condition the exception was written to expire on has
  ...
  already asserted for this class. Reported and not taken, read out of the fetched
...
- 📝 **The charter's version-chain example prints the chain that ships, and an arm holds it there.**
  ...
  It is corrected by addition, not by replacement, because the 0.0.79 chain is a true
  record of what `launch.ps1` resolved on the day the argument was made, and the argument is
  about the shape of the chain, not about any link in it. The chain as it ships now
  stands beside it, read from the payload lock and the committed `browsers.json` snapshot,
  not from memory: `@playwright/mcp` 0.0.81 -> `playwright-core`
  ...
  variation the old chain could not show, and the text says so instead of leaving a reader
  ...
  stopped matching reports that it can no longer tell instead of reporting nothing; and a
  reworded anchor, which throws and says to re-anchor and not to delete. The wrapper's
  ...
  they record what entries were measured under; rolling one would falsify the record and
  not update it.

  `BrowserAiPaths.BrowserVersionOf` joins `RevisionOf` beside it, so the suite still has one
  reader of that snapshot, not two. A revision bump at an unchanged browser version
...
- 📝 **The README licensing tables name Firefox, the third Playwright package, and today's revision.**
  ...
  Every cell is corrected by addition from the licensing re-read of 2026-09-17, not
  ...
  What it cannot read is the prose inside a cell, and the README says so instead of
...
- 📝 **The session ledger is snapshotted again, and a re-snapshot is not an edit.**
  ...
  longer whole. That was checked and not trusted -- the previous body is a byte-exact
  ...
  re-snapshot, not the edit, which is why a ledger sits outside the append-only
  record test.
...
- 🗑️ **The idle-CPU axis is retired as not established, and the other three are unchanged.**
  ...
  One thing is established and is kept and not retired: Firefox does not burn an order
  ...
  -- a longer window, or CPU sampled and not differenced, on a machine with nothing
  else running -- and not another six rounds.

  Nothing consumes the axis, and that was checked and not assumed. The four ratios
...
- 📝 **The third-party notices name the third Playwright package and both browsers we provision.**
  ...
  what was missing was a name, not a licence. It is in the table now with its three
  ...
  payload lock and not typed, so a roll is red until this file has been read against the
  tree it describes.
...
- 📦 **The release manifest records the pulled-forward dependency, and copies the file that pins it.**
  ...
  force and a block per overridden package when one is. Every number in it is read and
  not typed: the shipped version and each package's declared version out of the payload
...
- 📝 **Resume is two paths now, and each was timed at 1245 and 1548.**
  ...
  is the first evidence, not argument that a resume is about the directory and not
  about the browser.
...
- 📝 **The cost ratios are re-taken at 1245 and 1548, and the idle-CPU axis is withdrawn.**
  Re-verification row 34, at six rounds per family, not the stated three, because
  ...
  conclusion about the instrument, not about either browser: Chromium's six rounds
  ...
  it; what to do about the axis is left open and not decided.
  ...
  Chromium's own column between the two runs measures the rig, not the browser --
  ...
  evidence for this section's own claim that the ratio is the transferable half,
  not an assertion of it. The procedure gains a sentence with it: both families must run
...
- 📝 **Four claims in the payload licensing row did not survive a re-read at 1245 and 1548.**
  ...
  half could not have moved and the reason is evidence, not an assurance:
  ...
  `playwright-core`'s, so a name is missing, not a licence, and that is reported
  and not fixed because what ships beside the binary is the maintainer's call. And
...
- 📝 **Disk after a first run is a measured number again: 598 MB.**
  ...
  weighed as the thing the sentence describes and not reconstructed --
  ...
  total, not a defect in this one.
...
- ⬆️ **TUnit moved 1.68.4 -> 1.68.17 and this time the testing platform moved with it, 2.4.0 -> 2.4.1.**
  ...
  the number it lands on is upstream's choice, not ours.
...
- ⬆️ **The payload rolled to `@playwright/mcp` 0.0.82, and three browser revisions came with it.**
  ...
  which is one Playwright version in the payload again, not two. The browsers moved
  ...
  EVERY MACHINE RE-PROVISIONS, AND THE COST WAS RE-MEASURED AND NOT CARRIED FORWARD.
  ...
  third roll running** -- `cftUrl()` is keyed on `browserVersion`, not on the
  ...
  not a rebuild**: 130,934,199 B on the wire (+1,431,551) and 365,579,913 B
  ...
  were measured and not read. `tools/list` is no longer a static surface: the child
...
- 🗑️ **The dated `playwright-core` override is removed, four days after it was taken.**
  ...
  its course is a worked example of how one is allowed to work, not a slot that
...
- ✅ **The exit test, its positive control and the version ordering underneath them are retired.**
  ...
  **Deleting a test for a mechanism that no longer exists is not a skip**, and each
  deletion is recorded where the test was referenced and not removed without trace:
...
- 🔒 **A web page can add tools to the child's `tools/list`, and BrowserAI's surface does not move.**
  ...
  **a web page invented**.
  ...
  WHAT IS NOT CLOSED IS THE TEXT, AND IT IS REPORTED AND NOT DECIDED. Tool results are
...
- 🔧 **Two tools left the surface and nobody here judged them out.**
  ...
  REASONING IS PRESERVED AND NOT LOST**, in `tool-verdicts.json` and
  ...
  was re-counted off the regenerated snapshot and not decremented.
...
- 📝 **Four re-verification rows are marked `[STALE]` and owed, and the reason is on each.**
  ...
  Each needs a measurement session, not a read, and **none was adjusted from the
...
- 📝 **The session ledger is snapshotted for the last time, and the scratch folder is
  ...
  **What makes this one final is that the live copy is gone.** `.work/` is ephemeral by
  ...
  a new session opens a new ledger beside it instead of appending to a closed one, because
  appending from a different session's live file would rewrite a body and not extend
...
- ✨ **BrowserAI ships as two executables from this release.** The reason is a window nothing could
...
- ✨ **`BrowserAI.exe` is the configuration app, and it opens after the install.** It is the Velopack main executable, the root stub, the
  ...
  **Nothing runs on open except reading state**, and a first run shows what
  happened instead of asking anything.

- ✨ **The two registration scopes are offered in OutlookAI's own words.** Two products in one
...
- ✨ **`BrowserAI.exe --report <path>` writes a JSON status report and exits.** It
  ...
  report that disagreed with the screen would be a red, not a discovery.

- ♻️ **`BrowserAI.Core` is the library both executables share.** The data root, the registration, the update
...
- ✨ **`Runtime/PeSubsystem` reads a binary's subsystem out of its own PE headers.** Eight bytes read at three documented offsets, no Win32 -- and
...
- ✅ **`RealInstallerTests.TheInstalledMainExecutableOpensOneDialogAndNoConsoleWindow` is new.** The arm the whole design was cut for. It installs silently into a scratch
  ...
  polls instead of sleeping once: two runs of the same probe by hand disagreed
  ...
  read out of the binary -- the cause, not the symptom.

- ✅ **`ConfigurationAppTests` covers the configuration app.** It asserts every sentence, link and button of the dialog without opening
  one, and
  **`BuildConfigurationTests.TheConfigurationAppDeclaresTheVersionSixCommonControls`**
  refuses an app manifest without `Microsoft.Windows.Common-Controls`
...
- ✅ **`HouseRuleTests.EveryArmInAFileThatOverridesTheEnvironmentRunsBesideNothing` is new.** A tree-as-text scan holding that every arm in a file under `tests/` that
  constructs an `EnvironmentScope` carries `[NotInParallel]` **with no key**. A
  keyed one does not satisfy it. **The file is the unit, not the arm**,
  ...
  another arm's browser launch, which is a scheduler outcome, not a call,
...
- ✅ **`SaturationTests`' torn-record arm is scoped to the run's own pids.** A second arm plants the
...
- ✅ **Every run says whether it could have seen a browser take the foreground.** Because on this machine it could not. `JobLauncher` sets
  ...
  **It reports and it never repairs.** Nothing calls
  ...
  from `TestDefaults.BrowserHang`** instead of being written at the comparison,
...
- ✅ **The dated records are append-only, and a test says so.** Released
  ...
  **It is deliberately narrow.** `docs/reviews/README.md` is *not* sealed -- it
  ...
  line in the diff, not a silent rewrite. [Release checklist item
...
- ✨ **Six per-run arguments, and four opinions that stopped being arguments.** `viewport`,
  ...
  **4,784 -- exactly the per-image cap, with zero headroom**. What is set is what
  ...
  that does not parse or is out of bounds is **refused and not rounded**,
  because a size a caller did not choose is one every later screenshot is
  silently taken at.

  **`locale` and `timezone` are read from the host machine** and not
  ...
  where that conversion is unavailable the key is **omitted and not
  guessed**, because a Windows identifier fails the launch instead of
  degrading.
  ...
  **`captureNetwork` writes an HTTP Archive, and sets `serviceWorkers: "block"`
  ...
  launch, so the overwrite-on-resume is avoidable, not documentable. The
  ...
  every Firefox session **unusable**, not degraded. Measured 2026-08-20
  ...
  **`codegen: "none"` and `snapshot.boxes: true` are hard-coded with no
  ...
  carries a link, not the text -- and every session is granted the
...
- ✨ **`browserai_catch_up`, the seventh authored tool.** It answers *what were
  ...
  routinely disagree** -- which is the whole point, not a caveat. The
  ...
  *navigation*, not from tools, so a session whose log shows no
...
- ✨ **One time-ordered log, inside `browserai.json`.** `browserai_init`'s
  ...
  purpose here* sitting between the calls it explains, not two streams
  nobody merges. `browserai.json` moved to **schema 4**; there is no converter
  and the recovery is the one it has always carried.

  **It is inside the record, not in a sibling append-only file, and that
...
- ✨ **A required `why` on every call that names a session.** Every upstream
  ...
  mutates the `JsonNode` the child sent instead of rebuilding it, so unknown
  ...
  **The description does the steering, and parameter descriptions are
  uncapped**, so the long text is there, not in the tool description that
  is capped at 2,048 characters. It asks for **why, not what**, because
  ...
  not only that something is missing, because a model told *"'why' is
...
- ✨ **Ten tools, reachable for the first time in this product's history or its
  ...
  **Recorded as a deliberate grant and not left as a consequence.** They
...
- 📝 **A warning in the server `instructions` that response mocking can make a page
  ...
  the agent made up. **It is in BrowserAI's own string and not appended to
...
- ✨ **`browserai_list` now says whether each session it reports is being driven,
  ...
  **Through the pre-gate probe, `SessionLock.ProbeLiveness`, and never the
  ...
  open the peer cannot defeat it, and it asks about the resource, not
  ...
  filters, not walks, so a drive-root listing adds one open per known
...
- ✨ **Liveness is three-valued: `Alone`, `NotAlone(n)` or `Undetermined(why)`.** `LiveInstances`
  ...
  anything that repairs instead of refraining it is the unsafe direction, because
  ...
  **The updater did not move, and that is a guarantee, not an
...
- 🔒 **A reinstall takes the machine's browsers root for the whole call.** `browserai_init` and `browserai_resume` are refused while it holds it.
  ...
  **The claim is a lock file at `<browsers root>\reinstall.lock`, held open
  `FileShare.Read` for the whole operation.** A file, not a named mutex
...
- 📝 **`--storage-state` together with `--user-data-dir` is a silent no-op.** Exit 0, empty stderr, no
...
- 📝 **Two browsers on one profile directory: Chromium refuses and Firefox hangs.** Chromium refuses in 5,036 ms naming the
  ...
  anything spawns; upstream's bug is recorded and not worked around.

- 📝 **The user agent is settable from the config on both families.** `navigator.webdriver`
...
- 📝 **Firefox is owed Chromium's rename measurement, and now has it.** Plus the two shared
  ...
  **identical to Chromium, error for error and in the same order**, over two
  ...
  not a generalisation from one. The Chromium arm was re-run first and
  reproduced exactly, so the record is reproducible from what was written,
  not only from the day it was taken.
  ...
  **freely** while a browser of either family is live -- neither is running,
  ...
  its image sits in. Recorded and not acted on, and explicitly not a licence
...
- 📝 **The CsWin32 metadata licence terms are in the repository, quoted verbatim.** The
  question is now a legal one, not a research task. The
...
- ✨ **`browserai_reinstall_browser` gained a third value, `shared`.** It is the only route to repairing
  ...
  **both** families, each carries its own `INSTALLATION_COMPLETE`, and a family
  ...
  run that exits 0 having left one unmarked is reported here and not met
...
- ✨ **Firefox is a browser you can ask for.** `browserai_init` accepts `browser:
  ...
  **The tool surface does not depend on the family.** Measured the same day
  ...
  claim about both families, not an assumed one.
...
- ✅ **A budget gate over every model-facing string, measured off the wire.** `ModelSurfaceTests.EveryModelFacingStringFitsTheClientsSilentTruncationBudget`
  ...
  the entry below -- so the gate now fails on characters, not on whichever
  count is larger.
...
- 🐛 **The per-directory session gate refused sessions that nothing was wrong with.** It told the caller so in as many
  words. `LockScopes.PerDirectoryGate` was five
...
- 🐛 **`browserai_reinstall_browser` could delete the browser tree and wait an hour.** The hour is spent with nothing installed.
...
- ✅ **Both intermittent failures now name their own state.** The race probe
  ...
  reports, not the one that tripped. `JobObjectScope.SaidBy` waited for
...
- ✅ **CI's four skipped tests are settled and not rediscovered.** [`TESTING.md`](TESTING.md#continuous-integration)
...
- ✅ **The `#anchor` half of every relative link is checked too.** The link test
  ...
  which is itself asserted against worked examples and not trusted.

- 📝 **`Interop\`, `Sessions\` and `Runtime\` have working instructions of their own.** Twelve
...
- 📝 **This changelog.** And `build/Get-ReleaseNotes.ps1`, which
...
- ✨ **The running build's version is the first line of every process log.** So *"which
  ...
  demonstrated and not asserted: after a real update and a real rollback,
...
- 📦 **`build/New-Release.ps1` builds a release end to end.** It publishes, packs and refuses: on a `vpk` that does not match the
...
- 📦 **Velopack's MIT licence and a trademark disclaimer now ship inside the package.** They are in
...
- 📦 **A release now records the resolved set beside its artifact.** It is emitted and not
...
- 📦 **BrowserAI has an icon, and it is candidate 3 of the ten drawn on
  ...
  rasteriser and not downscaled from the 256. The master is persisted beside
...
- ✅ **Nothing in a release body may carry a character a person would not type.**
  ...
  because a span quotes something that exists instead of choosing a style. The
  scope is what a release body is actually made of and is stated and not
...
- ✨ **Every file a tool result names is now named absolutely.** A screenshot,
  ...
  result is a model, not a process with a working directory, so that
  ...
  One shape was not driven and is recorded as owed and not claimed: the
  paused-debugger location, which needs a paused session to provoke. The key is
  written explicitly and not left to a default, because upstream's default
...
- 📝 **The gate now says to publish the slice again after a `src/` change.**
  ...
  NativeAOT binary, not the tree, and `PublishedSlice.EnsureFresh`
...
- 📝 **The disk total after a first run is stamped stale and not left reading as current.**
...
- 📦 **A pack built for the gate is not a release, so it is not given a release body.** `New-Release.ps1`
  ...
  Found by running the script, not by reading it: the pack succeeded and
...
- 📦 **The GitHub release body is generated from the section and not cut out of it.** The
...
- 📝 **Every entry in this changelog is an icon, a one-sentence headline and a fold.** **236
  ...
  mechanically over every entry and not by reading. The groups are the
  ...
  **The palette is a legend at the top and the icon is chosen for what the entry
  ...
  100-character budget is chosen and not measured and says so**: where
  GitHub wraps is a question about a browser's layout at a font size, and the
  markdown API returns HTML, not a line box.
...
- 📝 **`RELEASING.md`: moving a tag does not draft the release; deleting one does.** Corrected
  ...
  GitHub re-resolving `releases/latest`, not a drafting.

- 📝 **`TESTING.md` says to wait for `.work\test-scratch` to clear between two suite runs.** The wait is for the directory to be released,
  not for the first run to report. On the 2026-09-15 release gate, **137** rig
...
- ✅ **A suite race the packed release let loose reddened arms that did not hold the variable.** The arm holding the variable was
  ...
  **every arm in a file that constructs a scope**, which is `RealInstallerTests`
  and `RegistrationTests`, both on the class so a later arm inherits it instead
...
- ✅ **The same race had a second face, it passed, and nothing would have said
  ...
  that real shape, not on `browserai-*.log`, which would have exempted
...
- ✅ **Both halves of the race fix were watched against a live reproduction of it.** That is stronger evidence than either plant. One full suite run on 2026-09-15
...
- 💥 **The MCP server is `BrowserAI.Server.exe`, and older registrations name a file that is gone.** Every registration written before
...
- 🔧 **`RegistrationTarget` composes the server's path instead of copying its own.** The guarantee that
...
- 🔧 **The configuration app clears `VELOPACK_FIRSTRUN` and `VELOPACK_RESTART` from its environment.** This is a defect the two-binary design creates. The
  ...
  once after being read, instead of filtered at each launch site: there is more
  than one place this process starts something, and a filter that has to be
  remembered at each of them is the shape of the defect, not its fix.
...
- 📦 **`build/New-Release.ps1` publishes both projects into one pack directory.** HALT-A runs once per
...
- 📦 **`assets/BrowserAI.ico` is wired into the pack and into both executables.** The Setup stub, the
...
- ✅ **`ChangelogTests` was red on every release commit, and the check was what was wrong.** `ChangelogTests.TheChangelogHasAnUnreleasedSectionWithEntriesInIt`
  was red on every release commit, by construction, and it was the check,
  not the tree that was wrong. Stamping a release moves every entry under the
  ...
  version is a disagreement, not an exemption and stays red, and the
  accepted refusal still has to be the empty-section one **and** the entries
  have to have moved and not vanished. **Without git there is no
  ...
  **Watched both ways on this tree**: red today at `97661d4`, whose
  ...
  synthetic controls run unconditionally instead of inside the branch they
  describe, which would have made them unfailable on every day but one.

- 📝 **[`RELEASING.md`](RELEASING.md#the-release-gate) states the order of the last six steps.** Running them in the
  ...
  order, not defects**: stamping the changelog empties `[Unreleased]`,
  ...
  **refuses** an equal version instead of calling it monotonic, so a re-pack
...
- 📝 **[`README.md`](README.md#status)'s test-count sentence, re-measured with a release pack present.** That is the
  arrangement in which the two pack-gated arms run and not skip.

- 📝 **Three documents said this product had never been distributed, and it had been.** It was publicly downloadable for a month. `CLAUDE.md`,
  ...
  not a person. **All three were corrected by addition**, because a
  reader who learned the old sentence has to learn it was reviewed and found
  wrong, not find it quietly gone. **The 2026-08-24 correction was
  ...
  obligations attached on 2026-08-17, not today**, and whether the
...
- ⬆️ **`@playwright/mcp` 0.0.81 and `playwright-core` 1.64.0-alpha-2026-09-14 adopted.** One day after
...
- 🔧 **The generated child config now writes `timeouts.idle` explicitly.** The value is upstream's
...
- 🔒 **BrowserAI refuses to start when its *install* root is outside your Windows profile.** The same is true of its data root. The live-instance census -- which is what decides
  ...
  is now refused and not accepted silently.
...
- 🔧 **BrowserAI no longer keeps running when the installer starts it.** A
  ...
  BrowserAI whose launcher has gone **and** whose input is a console, not
...
- 💥 **Every machine re-provisions its browser on first run after this.** Two
  ...
  refused and not reused. **A rollback re-downloads too**, which is the half
...
- ⬆️ **Upstream now honours `chromiumSandbox` from a config file.** It deleted the CLI-stage
...
- ✅ **Every run now states the publish freshness it established, instead of only
  ...
  **01:20:40**, touching `src/BrowserAI/Sessions/SessionLock.cs`, went beside
  ...
  records when it ran instead of touching a working-tree file. Dissolving it
  ...
  the live arm asserts the tie in both directions instead of leaving it to the
  construction.

- 📝 **The six-run-gate hazard closed on the condition it wrote for itself.** It had re-opened on that same
  condition hours earlier. That row closed on 2026-08-24
  on a cause, not on a test, so it was obliged to name what would re-open
  ...
  its own cell instead of leaving a reader to work it out, and the re-opener it
  names is unchanged. The recorded same-slot observation is downgraded and
  not dropped: both stalls were the sixth run from Git Bash, and this set's
...
- ✅ **A stale-publish alarm was a commit date read as a file timestamp.** The
  ...
  *before* the publish. The batch edited, published, gated and committed in that
  ...
  block's `published slice PRESENT` row is a claim about existence, not
  ...
  state its own freshness margin is a change to the coverage block, not
...
- ✅ **A hazard row the parser could not read left both tallies in the same
  ...
  **the escape it recommends was made true and not suggested**:
  ...
  *counted* -- which is the second red, the tally going to 46 `open` against a
  ...
  skip to another. **The guard is a second walk of the table, not a
...
- ✅ **The failure dump could not read the files it exists to inline.** The one run
  ...
  **however permissive the writer was**: node's `fs.openSync` shares read, write
  ...
  the fix had to go. The close is on `'close'`, not `'exit'`, because
...
- 📝 **The six-run-gate hazard row is `open` again, on the condition it wrote for
  ...
  outside this repository entirely -- and because it closed on a cause,
  not on a test it was required to name what would re-open it: the same shape,
...
- 🔧 **A process re-taking a session directory it last held now says so.** It no longer reports a reclaim from a live stranger. `destroy` and `set_purpose` both
  ...
  *"previous holder was PID n, still running: True"*, which is true word by word
  ...
  and `HolderRunning: true` are answers about the directory, not about
  who is asking, and both are still right. What was wrong was the sentence.

- ✅ **The suite's coverage block records the machine's commit charge.** It is read at both ends of
...
- ✅ **The browser-containment driver reads its child's `stderr` instead of buffering it.** It spawned `cli.js` with all three streams piped
...
- 📝 **The silent Chromium death was reproduced on purpose, and desktop heap is the cause.** That holds for everything about it
  except its exit code. `QUESTIONS.md` §8 had been open
  ...
  is a property of the last four kilobytes, not of the shortage. The exit
  ...
  never a `1`, which is said as an unexplained gap and not smoothed over.
...
- 📦 **The release manifest says whether the release was a crunch override.** It always says something. `DECISIONS.md` stated in bold that *"a release whose
  ...
  half-stated override refuses the manifest instead of writing half an account
  of the decision.
...
- 📦 **The release manifest records seven files, not six: `tool-verdicts.json` is
  ...
  own `judgedAgainst` pair, read back out of the copy and not transcribed.

- ⬆️ **Every release builds against the latest Playwright, and the one override is
  a human's.** Written
  down and not assumed: a human may force a crunch override and **an agent
...
- 📝 **Adversarial and hostile-caller defence is a stated non-goal.** The premise
  ...
  Several arguments over the last week would have been a paragraph, not a
...
- 📝 **A just-answered call can read as still in flight, and that is now written
  ...
  caller never received -- and the window is a hazard row, not a fix.

- 🔒 **`browser_start_recording` and `browser_stop_recording` are judged `allow`.** The advertised surface
  ...
  instead of waiting for a human, every other tool answers normally while a
...
- 📝 **The server instructions told models something false about screenshots.** The correction hands them a lever they did not have. The `fullPage` sentence
...
- 📦 **`build/Build-Payload.ps1` sets `PLAYWRIGHT_SKIP_BROWSER_GC=1`, as the product always has.** The script never did. Rebuilding the payload for the 0.0.80 review ran upstream's
...
- 📝 **Two upstream asks were transferred by upstream itself.** The watch item that was reserving
...
- 🔧 **An aliased session directory is resolved and not refused.** The network refusal now
  ...
  **What is still refused is refused everywhere** -- at `browserai_destroy`,
  ...
  swept and not orphaned, and the next `init` or `resume` records it again.
...
- ⚡ **`browserai_list` no longer parses every session record on the machine.** The old walk opened and strictly parsed
  each one only to print the few under a prefix, and `init` and `resume` no
  longer do it either.
  [Adversarial review
  ...
  path, not only on a listing. `SessionIndex.FollowUnder` applies the
  ...
  **The predicate is unchanged and only its position is**, so the reported set
  is bit-identical; `IsUnder` moved with it instead of being spelled twice.
...
- 🔧 **The machine-wide log is one shared file under a cross-process write gate.** A
  ...
  **First, scope:** anything attributable to a session is written to that
  ...
  rate; it does not dissolve the contention**, and the lock, not the
  ...
  concurrent reader of the log. A file lock, not a named object for the
  ...
  not by anybody sorting it. **Every record now carries both of its
...
- 🔧 **Rotation happens exactly at the cap.** *Corrected in `RollingFileWriter`
  ...
  its own is written and not dropped, and lands alone in its own file.
...
- ✅ **The release gate's two shells are two instruments by construction.** A run
  ...
  red from the other. **That property held run to run and not by
...
- 📝 **`browser_get_config` DOES redact `secrets`, and three places said otherwise.** The claim that it does not is corrected in three
  ...
  **The same claim in
  ...
  day it was written, and correcting it there would destroy the account instead
  of improve it -- which is the rule the append-only seal above now enforces.
...
- 🔒 **BrowserAI refuses to serve out of an app root that is not inside the current
  ...
  enforced against handles, not tokens, but the **`Global\` mutexes do
  ...
  **The check runs before anything creates state** -- before the stray sweep, the
  ...
  is banned outright, and it carries the remedy, not only the verdict.

  **It resolves through the filesystem instead of comparing strings.** Both the
  ...
  *what does the filesystem call this*, not two -- and four arms of
  ...
  **It narrows the hazard instead of closing it, and the row stays `open`.**
  *Outside the profile* is not the same predicate as *shared*: a single-user
  ...
  and not refused, because a background MCP server that will not start on a
  locked-down machine is a worse failure than the one being prevented. All three
  gaps are stated on `InstallRootScope` itself.

- 🔧 **The claim on the browsers root is a reader/writer lock.** Every session holds it shared, and a
  ...
  *"I do not want the intent marker. If anything is busy then the reinstall
  ...
  through, and that is a decision, not a defect to be mitigated.
...
- 🔧 **Every refusal a caller meets during a long operation now says how far in it
  ...
  **The update refusal** -- an update downloaded, staged and not applied because
  ...
  not "another BrowserAI is running", distinguishes that from a census
  that could not be taken at all (a permanent block, not a queue, and it
  says why), and states that nothing more has to be downloaded, with the package
  size and the seconds it took.

- ✅ **The provisioning stall detector runs on an injected clock and byte source.** Its own flaky test is fixed by that, not by weakening it.
  ...
  **The assertion is stronger, not weaker**, which was the constraint. It
  ...
  anywhere in either arm**, so the flake is impossible, not unlikely.

- 💥 **A session's record is `browserai.json`, renamed from `lock.json`.** There is
  ...
  *"nothing is in production yet. The only version that exists is the alpha
  ...
  `browserai_list` and `browserai_resume` read it for those, not for
  ...
  **No fallback, no migration and no compatibility read**, which is the half
  ...
  build, and `browserai_init` will make a new record beside it instead of
...
- 🔒 **`allowUnrestrictedFileAccess` is set in every generated child config, always.** No argument can turn
  ...
  **Upstream calls it a convenience defence, not a secure boundary**, in
...
- 🔧 **Provisioning is stopped when it stops making progress, not when it has taken
  ...
  **Ten minutes is set by upstream's own lock, not by taste.**
...
- 🔧 **The refusal a browser call meets while provisioning runs is a progress
  ...
  -- labelled as arithmetic, not as a promise. The old sentence quoted the
  ...
  is now derived from a byte count and not hand-written beside one.

- 📝 **`QUESTIONS.md` had gone stale, and it is the document the maintainer reviews from.** So
...
- 🔧 **`browserai_destroy` returns `isError: true` when it could not remove everything.** The error
  ...
  **Every destroy assertion in the suite was re-read, not only the ones
...
- 📝 **A recorded hazard was measured and turned out not to be one.** Two family installers cannot
  ...
  **Measured five ways on 2026-08-19** and not reasoned from the source:
  ...
  **What is real is a wait, and it is now written down as one.** The retry
  ...
  and does not queue** -- loudly, recoverably, and inside the 45-minute
  ...
  per-executable loop is a red build, not a rediscovery.
...
- 📝 **Every row of the [hazard index](HAZARDS.md) is now adjudicated.** The count of the
  ...
  **The tally moved from [`TODO.md`](TODO.md) into the index itself**, because
  ...
  to be adjudicated when it is written down and not accumulated for a later
...
- 💥 **`browserai_reinstall_browser` now takes one required argument, naming the
  ...
  *Previously it took none, "because there is nothing to name: the install is
  shared by every session on this machine."* **The stated reason expired instead
  of being overruled** -- with two families provisioned there are two trees,
  ...
  instead of optional-with-no-default follows `mode` on `init`, this product's
  ...
  **The refusal narrowed with it.** The running-process check was always
  ...
  and not left to be discovered as one.
...
- 🔒 **A session directory on a network path is refused, and a mapped drive letter
  ...
  **22,210 ms**, and several such calls happen inside the per-directory gate --
  ...
  number for a drive letter, not for a UNC path
  ...
  **The guard cannot pay the cost it prevents.** The network question is
  ...
  object manager, not the filesystem -- `GetDriveTypeW` answered
  ...
  thing being avoided."* It was reasoning, not a measurement, and it was
  wrong.
...
- 🔒 **A second spelling of one session directory is refused, with the spelling to
  ...
  Refusal, not canonicalisation, taken as a decision: canonicalising
  ...
  ⚠️ **And 8.3 generation turned out to be per-volume, which CI found,
  not a document.** The developer machine shortens on its system volume and not
  ...
  **Both tools now say so to the model, and one sentence had to be corrected
  and not extended.** `browserai_init`'s description said *"Any path is
  ...
  describes. `ModelSurfaceTests` requires the fact, not the sentence,
  ...
  nothing to refuse. Measured and not assumed, and the review is corrected
...
- 📝 **The browser-reinstall row rests on a measurement of Chromium.** It does not rest on a
  ...
  control that makes the two refusals Chromium's, not the tree's. So the
...
- 📝 **`@playwright/mcp` emits no progress notifications at all.** That settles the first of the two
  ...
  *to* a tool handler, and nothing in `@playwright/mcp` or `playwright-core`'s
  ...
  not a failed search. **The SDK's fire-and-forget reordering is
...
- 📝 **`LongPathsEnabled` is recorded where the long-path guarantee is claimed.** `longPathAware`
  ...
  unknown named and not implied: nothing has run against `LongPathsEnabled =
  0`, and the product neither checks it nor emits a diagnostic that would name
  it.

- ♻️ **`CreateProcessW`'s two buffers are declared as spans, not as one
  `char`.** `ref
  ...
  indexer, not the `null` the API accepts, and nothing saying which of
  ...
  signature that presents as a plausible wrong answer, not as an error.
...
- 💥 **`lock.json` is schema 2: every field is an ordered list of statements.** Each statement is timestamped, and
  ...
  **The payoff is that `browserai_resume` stops refusing a copied directory.**
  ...
  from the error catalogue and not orphaned, and **BrowserAI now has zero
  confirmation flags** --
...
- 🔧 **A contender asks the kernel who holds a session before it queues for the
  ...
  waited behind the whole queue of peers, not behind one critical
...
- 📝 **The hazard index says what each step was, not where it stood.** `HAZARDS.md`'s
...
- 📦 **The SDK is forbidden from decorating the version string.** The ban is repository-wide. Left on, it
  ...
  instead of comparing them starts downloading the binary it is already
...
- 📦 **The RID section a restore writes into `BrowserAI.Core`'s lock file is
  ...
  the restore genuinely resolves**, not an artifact of one publish shape.
  ...
  is the dirty one instead of ending it, and **that is still the right way
  ...
  cost is named and not hidden -- `git status` shows the file modified after
...
- 📝 **First-run provisioning is measured again at the new browser revisions.**
  ...
  not on the revision, and 1245 carries the same 154.0.8037.0 as 1244,
  ...
  the machine on the day, not anything about a revision. The run is kept
...
- ✅ **The one suite arm that launches an unowned browser runs beside nothing.**
  ...
  not rare. The arm carries a keyless `[NotInParallel]` now: the key it
  ...
  that key. What it costs is measured and not estimated, three runs each:
  ...
  lock on, which is a property of the running rig, not of its text, and
...
- 🐛 **A browser call into a session whose server has gone comes back now.**
  ...
  read from the MCP SDK's shipped code and not guessed: it faults every
...
- 🔧 **`browserai_resume` repairs a session whose browser server has died.**
  ...
  its own refusal, and it says the session is still open instead of inviting a
  fresh init.
...
- ⬆️ **`playwright-core` is pulled one build ahead of the wrapper that pins it,
  ...
  exception and not absorbed. It is the maintainer's decision, and it is a
...
- 📝 **The resume wedge is measured, and nothing in the product bounds it.**
  ...
  five minutes of margin, not a number a probe felt like waiting: **the
...
- 📝 **The wild exit 1 has a name at last, and it is this product's own stray sweep.**
  ...
  machine-wide process log and not reasoned about:
  ...
  excluded by its own announcements, not by argument** -- the 2026-08-29
  ...
  browser is alive kills it: deterministic, not rare, a suite isolation
  defect, not a wild death, and posed as a question with three directions
  and a plantable red and not fixed here. The suite's own spawn record could
...
- 📝 **The 1.0.0 release note opens in plain words now.**
  ...
  release"*, not *"Every entry in full, with its evidence"*, which is a
...
- 🔧 **The icon legend is a compact table now, in the release body and in this file.**
  ...
  The release body carries this file's own legend, not one of its own,
  ...
  not a table is refused, not flattened**, which is the half that makes
...
- 📝 **Two records catch up: where the probe rigs live, and what upstream did with the first ask.**
  ...
  narrowed to read the filter, not the API. The blind spot is **one file
  ...
  to the alpha that carries it, not by waiting for `@playwright/mcp` to
  roll -- a dated exception with a written exit. The row stays open, and what it
  waits for is the review, not the roll.

- 📝 **Chromium stays the default browser, on a reason, not on four numbers that moved.**
...
- 📝 **The upstream record catches up: one ask granted, one fix declined, one
  ...
  API on 2026-09-17 and not carried over.
  **(1)** [microsoft/playwright#42497](https://github.com/microsoft/playwright/issues/42497)
  ...
  2026-09-16T15:38:22Z, not by a reply. The flag, the `filePaths` config key
  ...
  *"this is really an upstream issue and should be fixed there instead"* -- so the
  ...
  a browser-revision bump, not a `playwright-core` change. The settlement
...
- ✅ **A pid that vanishes between the walk and the query is *exited*, not
  *unknown*.**
  ...
  reddens the run, so this is a classification, not a retry or a
  ...
  the error numbers *measured* and not quoted. [`HAZARDS.md`](HAZARDS.md)
  carries the row.
...
- 🗑️ **`browserai-sessions.json`, the per-root roll-up, and every mechanism that
  ...
  *parent* of the session directory it was given, so a session at
  ...
  this file -- and the walk that stays returns directories, not a record
...
- 🗑️ **Continuous integration, completely.** `.github/workflows/build.yml` was
  ...
  `.github/` is gone entirely and not left as an empty husk. **The whole
...
- 🗑️ **`SuiteCoverageTests.TheWorkflowStillDeclaresWhatItExpectsToBeAbsent` is deleted.** The capability pin's third arm. It read `build.yml` scoped to the step that
  ran the suite, so deleting the declaration was a red build, not a
  silent switch-off. **Deleted and not re-pointed, and the reason is a house
...
- 🗑️ **`BuildConfigurationTests.NoSourceFileIsInvisibleToGit` is deleted, deliberately.** This entry exists so nobody re-adds it believing
  ...
  to widen it past `*.cs` instead of remove it** -- *"I do not think we need
  this test at all."* Gone with it: `TrackedFilesAsync`, the harness that
  shelled out to git and served nothing else.

  **What is now unenforced, said plainly and not left to be found.** Nothing
  ...
  *produces*. **64 unanchored directory rules remain** in the upstream half of
  ...
  and not corrected.* Both `.gitignore` comments that named the test now
...
- 🗑️ **`browser_annotate` is gone from the model-facing surface.** Not refused --
  ...
  **A caller that names it anyway is refused, not forwarded**, because a
  ...
  **What went with it**, instead of being left unreachable: the mode-keyed
  ...
  **every upstream description now passes through byte for byte**, which is a
  ...
  from upstream's bundle, not from what this build calls.

- 📝 **The sweep's two highest-value assumptions are measured.** Both decisions they were holding up
  ...
  **foreground** within 1.2 s every time, and in the control arm the call was
  ...
  and writing its profile into `%TEMP%`, not into any session directory.
...
- 📝 **The justification sweep: 598 load-bearing reasons sorted.** 63 assumed, 13 settled
  ...
  **Relabelled and not settled**, because an admitted gap beats a confident
...
- ✅ **The client's *"2KB each"* is per string, and the gate was measuring the wrong
  ...
  API -- what the model receives, not what a model recalls. **Per string,
  ...
  delivers whole -- the parameter cap is relabelled a **house limit**, not
...
- 🗑️ **BrowserAI advertised an MCP capability it does not implement.** The SDK's
  ...
  not only that `tools` is present.
...
- ✅ **`HazardIndexTests.EveryRowIsOpenOrClosedAndNothingElse` enforces its own name now.** It did not enforce the invariant its name promises. It asked whether the `Status`
...
- 📝 **Four counts in prose were wrong.** One of them is a correction that had
  replaced a right number with a wrong one by measuring a different predicate
  over the same table. `TODO.md`'s hazard
  ...
  installed payload, not the installer: re-measured off the artifact
...
- 🗑️ **The free-space check is gone, and nothing asks a volume how much room it has.**
  ...
  **640 MiB**, naming the number, before it created anything.
  ...
  not 26. **The removal was planted red first**, two ways: the same
  ...
  *not* to refuse (*"Expected to not be equal to True but received True"*), and
  ...
  before the change and none after. **The old refusal test was deleted and
  not skipped**: it asserted behaviour that no longer exists, which is not a
  gap in coverage. **What it costs is written down and not implied** -- a
...
- 🐛 **A launcher that had exited but whose pid still opened was read as a live client.** `ClientLivenessWatcher`
  ...
  **every** time in under 400 ms, not once in four. Both arms wait for
...
- 🐛 **BrowserAI 1.0.0 did not exit when it had nobody to serve, and every install left orphans.** Every non-silent install
  ...
  and a loop the peer will never end is **abandoned** and not awaited: safe,
...
- ⚡ **A run with nobody to serve started a browser server before working out nobody was there.** It worked that out 506 ms later. On the same install, the `playwright-mcp` child
  ...
  **before** the no-client question was asked -- so the run with the least reason
  ...
  child. The watch itself is attached there instead of a second predicate being
...
- ✅ **The suite's installer arm destroyed the maintainer's Add/Remove Programs entry.** It did so on every run that had a pack to install. Velopack writes one uninstall key per
  ...
  scratch root is `Real`, asserted over constructed inputs, not by
...
- ✅ **The release gate's ILC check could not fail.** `IlcCompile` is an MSBuild
  ...
  **389** for the release script's own publish into a cleared output directory.
  ...
  not a line count: `Generating native code` is ILC's own line.

- ✅ **Release checklist item 8's code fence set the release variable and nothing else.** It handed `dotnet test` no explicitly-spelled absolute
  ...
  quoted in place and not deleted.

- ✅ **The harness's process-log reader answered with a stranger's records.** It
  matched half an identity. `ProcessLogRecords` is what lets a test assert *the
  product recorded X* against the durable file, not against stderr, and
  ...
  *begins* with the right one is not a match either. **The pid-only entry point
  is gone and not caveated**, because a reader that can be handed half an
  ...
  and not watched live, and that is a property of the subject, not a
...
- ✅ **The suite's reclaim pass could terminate a live run's processes machine-wide.** It did so with exit code
  1, and left no record that it had. `.work\spawn-record.txt` is
  how a killed run's leftovers are named for the next one, and the pass that
  reads it runs on first use of a scratch root **in each process**, not
  ...
  given child. Rows the pass declines are **written back verbatim** instead of
...
- ✅ **The guard on the `[STALE]` marker forbade the one resolution its own failure
  ...
  **backticked** marker, which is how the conventions table spells all five and
  ...
  clause, not against zero, so **stamping an entry and moving that
...
- 🐛 **An idle browser close left no trace in the only record there is.** The
  ...
  names the timer instead of borrowing a caller's voice. **A record that will
...
- 🐛 **An unjudged `browserai_` name was refused without being recorded.** The
  ...
  still writes nothing and cannot; that residual is asserted and not
  described.
...
- 🔒 **A refusal echoed the caller's own control characters back into the model
  ...
  `RecordText.Escape`, which **shows** a code point instead of stripping it -- a
...
- 🐛 **A session directory too deep to hold its own `browserai.data` was accepted, created and locked.** It then failed with a message about the browser and a
...
- 🐛 **A duplicate tool name in `tool-verdicts.json` exited naming neither the file nor the row.** The duplicate was inside one half of the
  file, and what it exited was the process. The loader checked for a
...
- 🐛 **`browserai_list` on a drive letter with nothing mounted on it answered "no sessions".** True,
  and useless to a caller who typed the wrong letter. The empty answer now says
  whether the directory is there at all.

- 🔒 **Five `PLAYWRIGHT_MCP_*` variables are refused by name and not merely absent.** `ALLOW_UNRESTRICTED_FILE_ACCESS` first among them: it switches off the only
...
- ✅ **A suite arm lost the machine-wide sweep gate to a BrowserAI another test started.** It needed its own stray sweep to have run, and lost to a
  real BrowserAI once in five full runs. The sweep's pass may now be handed a patience, the suite's own
...
- ✅ **The re-verification index's gate now reads around a `previously "..."` clause.** It does so the
...
- 🐛 **`browserai_list` said `in use: no` about a session another agent was driving
  ...
  *busy* session's `browserai.json` is periodically **present and unheld**, and
  ...
  could not take is `UNKNOWN` with a reason, not `no`. The zero timeout
...
- 🐛 **Every failure to open `reinstall.lock` was reported as a reinstall in
  ...
  not a clause on the existing one: **two recoveries are two rows**, and
...
- 🐛 **A pinned artifact name was matched undelimited and never given back.** [Adversarial
  ...
  stated and not closed** -- a short, word-shaped name a page renders in
...
- 🐛 **A torn log record is no longer possible.** The machinery that made it possible is deleted and not
...
- 🐛 **A `browserai_destroy` racing a `browserai_set_purpose` leaked the session directory.** The leak lasted the life of the process. Nothing above
  ...
  reach one `SessionLock` concurrently, which is the design, not an
  ...
  **The interleaving is forced and not raced for, and the seams were already
...
- 🐛 **One junction above the install root made the stray sweep structurally blind.** Nothing distinguished that from a clean machine. Every path BrowserAI composes
  ...
  **every process missed, on every pass, for good** -- `candidates=0` forever,
  ...
  Measured, not reasoned about, on 2026-08-24 and for the first time: a
  ...
  **warning**, not only a census number -- every one whose spelling it could not
  ...
  Two residues are named and not left to be rediscovered. A **symlinked
...
- 🐛 **The instance directory's liveness rested on one child.** The blast radius was every
  ...
  **BrowserAI now holds a marker inside its own instance directory** --
  ...
  root's maintenance claim already use. It is taken by BrowserAI, not by
...
- 🐛 **The pointers BrowserAI handed the model did not resolve.** Two of them, and they were the
  ...
  **The fix is a mechanical rule, not a list of prefixes.**
  ...
  sorted, so this is a rule about pointers, not the sweep being switched
  off.

- 🐛 **755 stale `.live` markers had accumulated, and nothing ever reclaimed them.** The only code that reclaimed
  ...
  **Reclaim is now a routine of its own and runs from two places, both with the
  ...
  proved with a positive control and not argued:
...
- ✅ **The failed-rewrite recovery of `lock.json` has a test.** A rewrite drops
  ...
  asserted from the kernel, not from the object under test: a stranger's
  ...
  and not hand back something that reports ownership it does not have.

- 🐛 **The two ungated `lock.json` readers that acted on an absence are closed.** They ACTED instead of reporting
  it, and closing them took one pass each. An adversarial review enumerated thirteen
  ...
  **`SessionIndex` deleted a live session's index entry.** A `null` record was
  ...
  positive, not a timing guess:** the temp is created before the rename
...
- 🐛 **Two BrowserAI processes wrote holder statements into one `lock.json`.** What let
  ...
  **The two ownership tests are deliberately unchanged**, and one of them meets
  ...
  `Contended`. That is narrower -- a wrong sentence, not a wrong owner,
  correct again on the next call -- and it is now a row of its own in the [hazard
  index](HAZARDS.md#hazard-index), not an unstated residue.

- ✅ **The suite was red from Git Bash and green from PowerShell on the same commit.** Now the wrong comparison is red in both. Two assertions in
  ...
  **Eleven sites carried the shape, and all eleven were test assertions** --
  ...
  names a caller chose. One of the eleven was choosing a *branch* instead of
...
- 🐛 **A durable write that landed said *nothing was changed*.** It could not be re-opened, and the sentence was
  ...
  meets on its next call as somebody else's crashed session, not as its
  ...
  not with a fault-injection seam in shipped locking code. This is the
  second, separable half of the interleaving recorded below; **the window itself
  is untouched and still open.**

- 📝 **Not fixed, and recorded loudly: two processes appended to one `lock.json` in CI.** They appended holder statements, on 2026-08-19. This is the interleaving the
...
- ✅ **A probe wrote its report in place, so `File.Exists` became true mid-write.** `File.Exists` was being used as the readiness signal. Found on 2026-08-19 by running the
  ...
  not the harness. ⚠️ **The sharing violation was the lucky half** -- a
...
- 🐛 **A screenshot comes back inline again, and the defect was ours.** Upstream
  ...
  **No size threshold, because upstream has none** and because a byte count is
  ...
  One divergence is recorded and not fixed: upstream shrinks anything over
...
- 🐛 **Closing the process log did not close its file.** Disposing the logging
  ...
  and the factory never disposes a provider it was handed and not asked to
...
- 🐛 **A `session.json` that could not be written was silent.** So was a session roll-up, while the same
...
- 📦 **Two licences that had to travel with the binary were not travelling.** `ModelContextProtocol`
  ...
  *linked in* and *its notice ships* are independent facts and the second was
  ...
  recipient, which is stricter than MIT's notice clause, not looser. All
  of it now ships in `THIRD-PARTY-NOTICES.txt` beside the binary. Upstream's MCP
  `LICENSE` turned out to grant **three** licences, not one -- Apache-2.0,
  ...
  **The `Microsoft.Extensions.*` list is derived from `packages.lock.json`
  and not typed**, so a package entering the closure on a later bump is a
  red build here, not a licence nobody noticed had arrived.
...
- ✅ **`UseSystemResourceKeys` is asserted and not merely set.** It strips
...
- 🐛 **Every session's `lock.json` would have recorded the wrong version.** The
  ...
  derived as. Measured on the artifact and not reasoned about: at `v0.1.0`
...
- 📝 **Six stale sentences are corrected and one new hazard is written down.** The
  ...
  *nothing to correct* on an update, both of which the two-binary split made
  ...
  now instead of measuring anything, so that procedure was broken and not
...
- 🐛 **The release-notes generator refuses the two shapes it used to crash on or
  ...
  line. Refusing instead of carrying the paragraph is the choice: prose already
...
- 🔧 **A wait on the client's handle that cannot be interpreted now records the
  ...
  logging, not a state of the client. The record carries the raw value
...
- 🔧 **The update check runs off the UI thread, under the same deadline the
  ...
  enforced by the poll, not by the token, because
...
- 🐛 **The folder picker is owned by the dialog, and an unresolvable folder
  ...
  states, not a nullable string: picked, cancelled, or failed with a
...
- 📦 **The suite's installer is titled `BrowserAI (suite)` and no longer
  ...
  elements, not two when it builds the second pack -- the id, the title
...
- 🐛 **Three invisible backspace bytes are gone, and one of them had silenced a
  ...
  now a measurement, not an artefact. The other two were prose: a comment
...
- ✅ **No text file in the tree may carry a C0 control byte.**
  ...
  which is why this is a mechanism, not a habit: a reader sees the escape
  ...
  is written to disk and not passed as a string so the reading half is
  exercised too.
...
- 🐛 **An "ours" registration that names the wrong binary is stale, not
  ...
  *is the MCP server*, read out of the file's own PE optional header
  ...
  answer to the question, not two. Anything else under our root is
...
- 📦 **A release body is one shape now: a headline a line, each linked to its own
  ...
  **The line numbers are only true of one file, so the generator refuses anything
  ...
  elsewhere is reported and not refused, because this runs inside
  ...
  scratch -- is a state, not a failure and the run says so. **The highlight
  was verified on github.com in a real browser and not assumed**: `curl`
  ...
  shares re-verification row 128 instead of taking one of its own.
...
- 📝 **A publish leaves `BrowserAI.Core`'s lock file modified, whichever publish
  ...
  the diff and not commit it, and say plainly that **nothing enforces it**: an
...
- 🐛 **A read-only file no longer defeats the delete every tree delete goes through.**
  ...
  unchanged and a genuine sharing violation is still reported and not
...
- ✅ **The dated dependency override cannot be forgotten: two instruments go red
  ...
  the two version shapes upstream publishes and refuses any third instead of
...
- 📦 **`New-Release.ps1` clears its own test feed before it packs into it.**
  ...
  and an absent or empty directory is reported and not refused, because the
...
- 📦 **The release checklist now clears the suite's feed as well as the real
  ...
  is written down there as the script owner's to take and not taken by a
  release executor.
...
- 🔒 **A path that would escape the session is refused with a sentence saying why.** It is
...
- ✨ **First-run browser provisioning that does not block the conversation.** `browserai_init`
  ...
  A run that fails halfway removes its partial tree instead of leaving
  something that looks installed.

- ✨ **Firefox as well as Chromium.** That includes the
...
- ✨ **Answers arrive exactly as the browser produced them.** The proxy splices
  the child's own bytes into the caller's frame instead of re-serialising, so
...
- ✨ **Failures say what happened and what to do next.** One error catalogue,
  ...
  condition and not asserted to exist.
...
- ⬆️ **Every dependency floats to latest and the build freezes what it resolved.** So the
  resolved set is recorded beside the artifact and not remembered.
