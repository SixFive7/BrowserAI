...
internal static class SessionToolSurface
{
    ...
    private const string WhyDescription =
        ...
        + "It goes in this session's log, which is what lets the next agent to open the directory -- or you, tomorrow -- read back what was being attempted, not only which tools ran.";
    ...
    private static IEnumerable<JsonObject> Authored()
    {
        yield return Tool(
            ...
            new JsonObject
            {
                ...
                ["tracing"] = Property("boolean", "Record this session into its output directory. A property of this run, not of the session; defaults to false."),
        ...
        yield return Tool(
            ...
            + "There is no unscoped form: breadth is stated, not assumed. Pass a drive root to see everything, and the size of the answer is then your own doing. The directory need not exist; a path with nothing under it returns an empty list, which is an answer, not an error.",
            new JsonObject
            {
        ...
        yield return Tool(
            ...
            "Closes the session's browser and deletes the whole directory -- profile, output, downloads and record. EVERYTHING IN IT GOES, screenshots and downloads included, so MOVE OUT WHAT MUST BE KEPT before you call this -- browserai_catch_up first, because the sizes are the only thing that says what you are about to delete. "
        ...
        yield return Tool(
            ...
            new JsonObject
            {
                ...
                [NameParameter] = Property("string", "The tool's name, exactly as the browser_snapshot block prints it -- character for character, spaces and punctuation included. Copy it; do not retype it. If the page gave the tool a display title as well, that title is NOT this: the name is what the block prints."),
                ...
                [PageParameter] = Property("string", "Optional, and worth passing. The page you read this tool on, exactly as the snapshot printed it after '- Page URL:'. If the tab has navigated since, the call is refused and both URLs are named, not running whatever the new page happens to call by the same name."),
                [WhyParameter] = Property("string", "Why you are calling this page's tool -- not what it does. One short clause: \"asking the page's own search for the order number, not driving its form\". It goes in this session's log beside the browser calls around it."),
            },
