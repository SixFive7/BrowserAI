...
`initialize` and `tools/list` rather than the constants they are assembled from.
...
| **What a destroy takes** | **Everything in the directory, screenshots and downloads included.** `browserai_destroy`'s description said *"deletes the whole directory"*, which a caller reads as a statement about the session rather than about the artifacts it was asked to produce. It now says to move out what must be kept first, beside the instruction — unchanged — to call `browserai_catch_up` before destroying, because the sizes are the only thing that says what is about to go. |
