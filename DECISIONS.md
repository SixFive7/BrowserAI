...
| **Automated checks** | ⚠️ ***Corrected 2026-08-19 (previously "**None. No CI, no scheduled job, no git hook.**"), and the correction is a fact rather than a re-decision.*** **There is hosted CI as of 2026-08-18** — [`build.yml`](.github/workflows/build.yml) builds the payload, provisions both browsers, publishes the NativeAOT binary and runs the **whole** suite on every push and pull request, `SaturationTests` included. There is still no scheduled job and no git hook. **What the row below decided is therefore now true of the release checklist alone**, not of the suite, and the decision it leaves open is narrower than it reads: whether the release-phase checks CI deliberately does not run — the packed release, the real client, `BROWSERAI_RELEASE_RUN=1` — should move into automation as well. Tracked in [`TODO.md`](TODO.md). The original text follows, unchanged, because what it argued is what is being re-opened. — The [release checklist](RELEASING.md) is the only gate that exists. That buys many commits without re-running everything on each of them, and keeps the project off hosted CI entirely. **The cost is recorded rather than softened: the gate works when it is invoked, and nothing makes it fire.** It is accepted because [the release trigger is already manual](#licence-release-policy-and-the-tool-surface) — the checklist runs at the one moment a human is deliberately doing something. **To be reviewed once the product is finished**, when the shape of the suite and the release cadence are known rather than predicted. |
