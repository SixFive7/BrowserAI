...
| **What ends it** | A human noticing. The watch is [`drift-check.json`](drift-check.json)'s `vendored` row, which names the issue and the glob that answers it | **A red build, on the day itself.** `PayloadTests.TheDatedPlaywrightCoreOverrideIsStillNeeded` off the committed lock, and [`build/Build-Payload.ps1`](build/Build-Payload.ps1) off the live resolution, each naming the file and the key to delete |
...
**The deletion is written down as a procedure, not left to whoever meets the red build.** [`TODO.md`](TODO.md#upstream-asks) carries a standing watch item — *WATCH for the `@playwright/mcp` release that carries #42497* — naming the condition, the exact object to delete, the rebuild, the review the roll then needs, and what to record here afterwards. Two things monitor it and neither is a person remembering: the [daily drift check](CLAUDE.md#the-daily-drift-check) resolves `@playwright/mcp` `latest` and its exact `playwright-core` dependency on every day of work, which is the read the condition is stated in; and the two instruments above go red the day it is met. **When the override goes, this section describes one exception and its heading says so.**
