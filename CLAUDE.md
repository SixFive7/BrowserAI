...
BrowserAI is a Windows-only, NativeAOT .NET MCP server that proxies a bundled `@playwright/mcp` child over stdio. [`README.md`](README.md) is the charter — scope and the reasoning behind every decision. [`ARCHITECTURE.md`](ARCHITECTURE.md) is how it is put together and which code satisfies which part of it. This file is the standing rules for working in the repository.

**Status: shipped at `v1.0.0`.** Work settled in intent but not yet done lives in [`TODO.md`](TODO.md); open design questions stay in the README; known failure modes are in [`HAZARDS.md`](HAZARDS.md). *Corrected 2026-08-17 (previously "design phase, nothing is built") — it was the first factual claim an agent read, and every rule below was read through it.*
...
Every dependency resolves to latest at build time and is frozen into the artifact. **Version numbers in the README, in [`STACK.md`](STACK.md#implementation-stack), in [`kb/`](kb/README.md) and in `upstream-review.json` are provenance stamps, not targets** — the build does not read them.
