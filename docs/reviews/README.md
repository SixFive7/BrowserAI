...
| processes **6** — the job handle crosses `CreateProcessW` with no ref-count | **real, recorded, not started, and the reason is unusual** — `GC.KeepAlive(job)` is still inside `ProcessAttributeList.For`, before the call rather than after it, and there is still no `DangerousAddRef`/`DangerousRelease` pair. The fix is six mechanical lines. **It is not taken because it cannot be planted red**: a handle-recycling race is not deterministically reproducible here, so it would be the first behaviour change in this tree with no failing test behind it. That trade is the maintainer's |
...
| processes **13** — `WaitForExitAsync` waits over a handle another thread may close | **real, recorded, not started** — hazard row. `ownsHandle: false` and an uncoordinated `Dispose` are both unchanged. **Same shape as 6**: the fix is small and cannot be planted red |
