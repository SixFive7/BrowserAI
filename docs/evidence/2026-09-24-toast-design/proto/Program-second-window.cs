...
internal static unsafe class Program
{
    ...
    [DllImport("user32.dll", ExactSpelling = true)] private static extern nint CreateWindowExW(uint exStyle, char* className, char* windowName, uint style, int x, int y, int w, int h, nint parent, nint menu, nint instance, nint param);
    [DllImport("user32.dll", ExactSpelling = true)] private static extern int SetForegroundWindow(nint window);
    [DllImport("user32.dll", ExactSpelling = true)] private static extern int DestroyWindow(nint window);
    [DllImport("user32.dll", ExactSpelling = true)] private static extern int PeekMessageW(byte* msg, nint window, uint min, uint max, uint remove);
    [DllImport("user32.dll", ExactSpelling = true)] private static extern int TranslateMessage(byte* msg);
    [DllImport("user32.dll", ExactSpelling = true)] private static extern nint DispatchMessageW(byte* msg);

    // Opens a small TEST window, asks for the foreground, and records whether it got it.
    private static void ForegroundProbe(string how)
    {
        nint window;
        fixed (char* cls = "STATIC")
        fixed (char* title = "BrowserAI Q254 TEST window (research, closes in 2 s)")
        {
            window = CreateWindowExW(0, cls, title, 0x10000000u | 0x00C00000u | 0x00080000u, 300, 300, 640, 160, 0, 0, 0, 0);
        }

        var before = GetForegroundWindow();
        var asked = SetForegroundWindow(window);
        var msg = stackalloc byte[64];
        var clock = System.Diagnostics.Stopwatch.StartNew();
        var marks = new[] { 0L, 300L, 1000L, 2000L };
        var next = 0;
        ...
        while (clock.ElapsedMilliseconds <= 2000)
        {
            ...
            if (next < marks.Length && clock.ElapsedMilliseconds >= marks[next])
            {
                result.Append($" t+{marks[next]}ms:{(GetForegroundWindow() == window ? "OURS" : "other")}");
                next++;
            }
        ...
        Log($"FOREGROUND-PROBE how={how} window=0x{window:X} foregroundBefore=0x{before:X} SetForegroundWindow={asked}{result}");
    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            if (args.Length >= 1 && args[0] == "fgcontrol")
            {
                ForegroundProbe("launched-from-a-background-shell");
                return 0;
            }
    ...
    private static int Activate(ComObject* self, char* appUserModelId, char* invokedArgs, UserInputData* data, uint count)
    {
        try
        {
            ...
            if (invokedArgs != null && new string(invokedArgs).Contains("review", StringComparison.Ordinal))
            {
                ForegroundProbe("COM-activated-by-a-toast-click");
            }
        }
        catch (Exception failure)
        {
