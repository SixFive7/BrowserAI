...
const t0 = Date.now();
const c = spawn(process.execPath, [__dirname + '/s-a.js'], { stdio: ['pipe', 'pipe', 'pipe'], env: process.env });
...
setTimeout(() => { console.log(`--- parent writes AFTER A's exit at +${Date.now() - t0}ms ---`); c.stdin.write('hello-after-handover\n'); }, 1200);
setTimeout(() => { console.log('--- parent done ---'); process.exit(0); }, 4000);
