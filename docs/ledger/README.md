...
⚠️ **A snapshot taken while its session is still open is INTERIM and says so in
its own header, and when the sweep above was applied to the copy and not to a
live file somebody is still appending to, that header names every edit, so the
next snapshot applies the same edits to the live file before it checks the
prefix** -- *added 2026-09-25, with the first snapshot swept on the copy only*.
...
| Ledger | Session |
|---|---|
| [`2026-09-24-development-session.md`](2026-09-24-development-session.md) | **OPEN** -- the session opened 2026-09-24, snapshotted once so far, on 2026-09-25 at commit 21de415, and that snapshot is **interim**: the night items (the 2026-09-24 drift check, Q261 against the real clients, T7 c and the Codex steps 2 to 6), the Velopack rows re-run at 1.2.158, the toast design questions (Q262 to Q274), the focus incident and the testbed batch that followed it, the coordinator design (Q280, Q282 to Q286) with its phases 1 and 1b, the history rewrite (Q295 a), phase 2 steps 1 to 5 up to their pause, and the research and open questions behind the zoom-out the maintainer asked for on 2026-09-25. *Its live copy is still `.work/STATE.md`, so a later snapshot replaces this one; the name is the day it was opened.* |
