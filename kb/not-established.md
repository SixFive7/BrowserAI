...
|---|---|---|
| Whether `--browser-test` is detectable **headful** | Not established | The whole fingerprinting comparison was headless. A hard constraint of the harness and not an oversight; nothing renderer-side depends on the switch, which is why it was accepted. [kb](chromium/fingerprinting.md) |
...
| Whether **any** vendor or anti-detect project references `--browser-test` | Searched and nothing found | **Absence of evidence, and it is labelled as such** and not as evidence of absence: five projects' code searched and four web searches. [kb](chromium/fingerprinting.md) |
...
| Which switch, if any, suppresses Chrome's *"Failed to create data directory"* dialog | Not established | `--noerrdialogs` does not, measured. No suppressing switch was identified, and the design avoids the dialog instead of dismissing it. [kb](chromium/profiles.md) |
...
| Whether `npm install` re-resolves a dist-tag dependency with a lock **already present** | Not established | The payload build deletes the lock first, so it never reaches that state. Open, not answered. [kb](toolchain.md#npm-for-a-vendored-payload) |
...
| Whether `ListToolsAsync(RequestOptions?, ct)` **still** drops a tool silently | Not established since the product stopped using it | Constructing an annotation that fails SEP-2243 validation has never been done here. The product's immunity is a source scan, not a measurement of the SDK -- and the row says so instead of reading as covered. [row 30](re-verification.md) |
...
| ~~The price of Azure Artifact Signing~~ | **CLOSED 2026-09-23** | Microsoft states it directly: *"Cost -- Starts at $9.99/month"*, Microsoft Learn, *SmartScreen reputation for Windows app developers*, fetched 2026-09-23. *Previously "Not established: ~$10/mo is a list figure, not a quote obtained."* The row stays as a closed row and not deleted, because what it records is that a figure this repository carried for weeks was never sourced. The same citation refutes the property the figure was carried FOR: reputation *"accumulates over time based on download volume and behavior"*, so nothing is bought on day one. [kb](packaging/velopack.md#distribution-msix-and-code-signing) |
| Whether redistributing Chrome for Testing is permitted | Not established, and **not answerable here** | The only on-point public statement names which terms apply and stops there -- *corrected 2026-09-23 (previously "is adverse and")*: the adverse reading belongs to the person who asked, not to Google, which never replied to it. It is a **citation, not a measurement, and not legal advice**. It is recorded because the provisioning decision rests on it. [kb](packaging/dependencies.md#third-party-payload-as-shipped) |

| Which Visual Studio version dropped the MTP discovery opt-in | Not established | *Use testing platform server mode* does not exist in Visual Studio Community 2026 **18.10.12210.168** -- 0 of 6,311 files under its TestWindow and Extensions trees carry the phrase, read as ASCII and UTF-16, against a *"Test Explorer"* control that fired in 29. Nothing older than that was read, and the search results naming 17.14 are secondary and were not confirmed against a Microsoft document. **What would establish it:** a Visual Studio release-note page that states it, or an older installation scanned the same way. Measured 2026-09-23. [HAZARDS](../HAZARDS.md#hazard-index) |
...
| **Which of Chromium and Firefox burns more idle CPU** | **Not established, and the axis is RETIRED** | *Added 2026-09-18 (Q215 = a).* Three readings, two sign reversals and the second inside one day: **~24×** in 2026-08, **0.77×** on 2026-09-16, **1.31×** on 2026-09-17. The distributions overlap completely -- Chromium 360-955 ms against Firefox 266-781 -- and the 2026-09-17 run carried the control that settles it: Chromium is **byte-identical across revisions 1244 and 1245**, so its own column between the two runs measures the rig, and on this axis it moved **−41%** against 0.9% on resident set and 0.0006% on profile disk. **An axis whose control moves 41% cannot resolve a 31% difference**, so more rounds of `TotalProcessorTime` differenced across one 30-second window will not settle it; re-opening it needs a longer window, or CPU sampled and not differenced, on a machine with nothing else running. **One thing IS established and is kept, not retired: Firefox does not burn an order of magnitude more idle CPU than Chromium**, which is what all three readings agree on and what refutes the recorded ~24×. **Nothing consumes the axis** -- the four cost ratios stopped being the ground under the default browser family on 2026-09-17 (Q206). The other three axes are measured and stand. [kb](playwright/provisioning-and-timings.md#firefox-against-chromium-the-standing-cost-ratios) |
| The method behind Firefox's cost ratios -- ~2× RAM, ~10× first navigate, ~24× idle CPU, ~20× profile disk | **Established 2026-09-17, and all four figures were wrong** | *Corrected 2026-09-17 (previously "Not established \| Carried from a session whose harness was not preserved. Order-of-magnitude guidance; re-measure before any decision turns on them.")* Re-measured through the product, three rounds per family, with the rig preserved at [`docs/probes/2026-09-17-cost-ratios`](../docs/probes/2026-09-17-cost-ratios/README.md): **1.19× RAM, 4.62× first navigate, 0.77× idle CPU** -- the reverse of the recorded sign -- **and 2.76× profile disk**. The row stays here as the record of what an unpreserved harness cost. ⚠️ **Three of the four were established and the fourth was not** -- *corrected 2026-09-18 by addition*: the **0.77×** above was re-taken as **1.31×** the same evening, a second sign reversal, and the idle-CPU axis is retired as not established in the row above instead of carried here as a figure. **This row's claim is unchanged**: the method behind all four is established now and the 2026-08 figures were wrong. What changed is that one of the four has no replacement value. [kb](playwright/provisioning-and-timings.md#firefox-against-chromium-the-standing-cost-ratios) |
...
looking, and not that somebody eventually looks.
