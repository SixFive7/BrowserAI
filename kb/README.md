...
table is how that happens. **It does not yet cover all of them**: **179**
`[FLOATS]` markers stand across the articles against the **75** numbered rows
below (82 lines, counting 2a, 2b, 4a, 4b, 4c, 16a and 63a),
...
|---|---|---|---|---|
| 1 | Playwright's restart command line overshoots 1023 by 531+, so **Chromium ends up unregistered** — and **Firefox registers anyway**, because its own pref does it independently of any length ([kb](chromium/resurrection.md), [kb](windows/processes.md#job-objects-and-process-containment)) | Playwright trims its arg list; or Mozilla changes `toolkit.winRegisterApplicationRestart`. The Firefox half is what [step 17](../plan/build-order.md#17-firefox) has to turn off in the profile, or a machine update resurrects a browser no session claims | `GetApplicationRestartSettings` on the **live** processes, never on a command line's length: every Chromium process must answer `0x80070490`, and exactly one Firefox process answers `S_OK`. Asked of both, so a change in either direction is red rather than quiet | `BrowserContainmentTests` |
