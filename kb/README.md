...
| [`windows/processes.md`](windows/processes.md) | Starting a process and talking to it -- stdio, exit codes, **third-party code that writes to stdout's *handle* from a type initializer**; durable writes, deletes and renames; the Win32 interop surface behind all three; **and, since 2026-09-24, the per-server named pipe** -- what a record rewritten in place costs a reader, what a pipe costs and what a client sees when one fails, and what crosses a process boundary in BrowserAI; **and what this machine starts first after sign-in**, what a process a logon task starts may do with the foreground, and a single instance through a mutex |
