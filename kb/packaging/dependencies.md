...
a new Firefox and not a rebuild, and **both `license.html` files grew by
...
> they reconcile exactly, which was measured, not argued. Under *files
...
> [the reason for first-run provisioning](../../DECISIONS.md) and not a side
...
exact build today; if you cannot, the recorded ID is an epitaph, not a
provenance. *(The Zeranoe shutdown date is a citation carried forward, not
something re-checked here.)*

**Why both belong in this article and not in the charter:** they are the
...
**Kept, not cut, and the reasoning is stated**, because both rest
entirely on repositories a reader cannot open. They survive because the claim is a
measurement -- git-log spans, byte counts, a shutdown date -- and not an
